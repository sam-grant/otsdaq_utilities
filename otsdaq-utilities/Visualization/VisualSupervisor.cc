...
VisualSupervisor::VisualSupervisor(xdaq::ApplicationStub* stub)
  ...
  theDataManager_ = DataManagerSingleton::getInstance<VisualDataManager>(
      ...
      supervisorConfigurationPath_, supervisorApplicationUID_);
...
//src='/WebPath/html/Visualization.html?urn=" << 			this->getApplicationDescriptor()->getLocalId()
//<<"'></frameset></html>";
//
//}
//
...
void VisualSupervisor::setSupervisorPropertyDefaults() {
  CorePropertySupervisorBase::setSupervisorProperty(
      ...
      "setUserPreferences |  getUserPreferences | getDirectoryContents | getRoot | getEvents");

  CorePropertySupervisorBase::setSupervisorProperty(
      CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.UserPermissionsThreshold, "*=1 | rootAdminControls=100");
}
...
void VisualSupervisor::forceSupervisorPropertyValues() {
  CorePropertySupervisorBase::setSupervisorProperty(
      CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.AutomatedRequestTypes, "getRoot | getEvents");
  CorePropertySupervisorBase::setSupervisorProperty(
      CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.NoXmlWhiteSpaceRequestTypes,
      "getRoot | getEvents");  // json data in ROOTJS library expects no funny characters
...
//========================================================================================================================
void VisualSupervisor::request(const std::string& requestType, cgicc::Cgicc& cgiIn, HttpXmlDocument& xmlOut,
  ...
  if (requestType == "getRawData") {
    ...
    if (fp) {
      ...
      fgets(line, 100, fp);
      sscanf(line, "%*s %s", val);
      if (radioSelect == "") radioSelect = val;

      fgets(line, 100, fp);
      sscanf(line, "%*s %s", val);
      if (autoRefresh == "") autoRefresh = val;

      fgets(line, 100, fp);
      sscanf(line, "%*s %s", val);
      if (autoHide == "") autoHide = val;

      fgets(line, 100, fp);
      sscanf(line, "%*s %s", val);
      if (hardRefresh == "") hardRefresh = val;

      fgets(line, 100, fp);
      sscanf(line, "%*s %s", val);
      if (autoRefreshPeriod == "") autoRefreshPeriod = val;
    ...
    sprintf(permStr, "%d",
            userInfo.permissionLevel_ >=
                CoreSupervisorBase::getSupervisorPropertyUserPermissionsThreshold("rootAdminControls"));
    xmlOut.addTextElementToData("permissions", permStr);  // add permissions

    std::string dirpath = rootpath + path;
    if (path == "/" + PRE_MADE_ROOT_CFG_DIR + "/") dirpath = ROOT_DISPLAY_CONFIG_PATH;
    ...
    if ((pDIR = opendir(dirpath.c_str()))) {
      ...
      if (path == "/") {
        if (theDataManager_->getLiveDQMHistos() != 0)
          xmlOut.addTextElementToData("dir", LIVEDQM_DIR + ".root");  // add to xml
        ...
        if (!pRtDIR)  // if doesn't exist, make it
        {
          ...
          if (mkdir(ROOT_DISPLAY_CONFIG_PATH,
                    S_IRWXU | (S_IRGRP | S_IXGRP) | (S_IROTH | S_IXOTH)))  // mode = drwx r-x r-x
        ...
        if (!recheck || (pRtDIR = opendir(ROOT_DISPLAY_CONFIG_PATH))) {
          xmlOut.addTextElementToData("dir", PRE_MADE_ROOT_CFG_DIR);  // add to xml
          if (recheck) closedir(pRtDIR);
        }
      }
      ...
      while ((entry = readdir(pDIR))) {
        ...
        if (entry->d_name[0] != '.' && (entry->d_type == 0 ||  // 0 == UNKNOWN (which can happen - seen in SL7 VM)
          ...
          if ((entry->d_type == 8 || (!isDir && entry->d_type == 0))  // file type
              && std::string(entry->d_name).find(".root") == std::string::npos && isNotRtCfg)
            continue;  // skip if not a root file or a config file
          else if (entry->d_type == 4)
            isDir = true;  // flag directory types

          xmlOut.addTextElementToData(isDir ? "dir" : (isNotRtCfg ? "dir" : "file"), entry->d_name);
        }
      }
      closedir(pDIR);
    } else
    ...
    std::string rootDirectoryName =
        rootFileName + ":" +
        fullPath.substr(fullPath.find(".root") + 5, fullPath.size() - fullPath.find(".root") + 5 + 1);
    ...
    if (!rootFile || !rootFile->IsOpen()) {
      ...
      if ((directory = rootFile->GetDirectory(rootDirectoryName.c_str())) == 0) {
        ...
        if (histo != nullptr)  // turns out was a root object path
        {
          ...
          std::string destination = BinaryStringMacros::binaryToHexString(tBuffer.Buffer(), tBuffer.Length());
        ...
        if (LDQM_pos == 0) {
          ...
          while ((obj = (TObject*)nextobj())) {
            ...
            xmlOut.addTextElementToData(
                (std::string(obj->IsA()->GetName()).find("Directory") != std::string::npos) ? "dir" : "file",
                obj->GetName());
          }
        } else {
          TKey* key;
          ...
          while ((key = (TKey*)next())) {
            ...
            xmlOut.addTextElementToData(
                (std::string(key->GetClassName()).find("Directory") != std::string::npos) ? "dir" : "file",
                key->GetName());
          }
        }
      }
      if (LDQM_pos == std::string::npos) rootFile->Close();
    }
    ...
    //		for(Visual3DEvents::const_iterator it=events.begin(); it!=events.end() && numberOfEvents < 10000; it++,
    //numberOfEvents++)
    //		{
    ...
    //				xmlOut.addTextElementToParent("xyz_point", str, eventParent);
    //				//__SUP_COUT__ << "X: " << itHits->x << " Y: " << itHits->y << " Z: " << itHits->z <<
    //std::endl;
    //			}
    ...
    //		{
    //			objectParent = xmlOut.addTextElementToParent("object", str, geometryParent);
    //			xmlOut.addTextElementToParent("object_type", "LINE_LOOP", objectParent);
    ...
    //			for(Points::const_iterator itCorners=itShapes->corners.begin(); itCorners!=itShapes->corners.end();
    //itCorners++)
    //			{
    ...
    if (cfgXml.loadXmlDocument(path)) {
      ...
      xmlOut.addTextElementToData("status", "Failed. File to properly load config file.");
  } else if (requestType == "rootAdminControls") {
    //		if(userPermissions < ROOT_VIEWER_PERMISSIONS_THRESHOLD)
    //		{
    //			__SUP_COUT__ << "Insufficient permissions for Root Viewer Admin Controls: " << userPermissions << " < " <<
    //ROOT_VIEWER_PERMISSIONS_THRESHOLD << std::endl;
    ...
    if (path.find("/" + PRE_MADE_ROOT_CFG_DIR + "/") == 0)  // ROOT config path must start the path
    {
      ...
      if (cmd == "mkdir") {
        if (mkdir(path.c_str(), S_IRWXU | (S_IRGRP | S_IXGRP) | (S_IROTH | S_IXOTH)))  // mode = drwx r-x r-x
          xmlOut.addTextElementToData("status", "Failed. Directory create rejected.");
        else
        ...
        if (rmdir(path.c_str()) == 0 || remove((path + PRE_MADE_ROOT_CFG_FILE_EXT).c_str()) == 0)
          ...
          xmlOut.addTextElementToData("status", "Failed. Target could not be deleted.");
      } else
