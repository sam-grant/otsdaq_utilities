...
void ConfigurationGUISupervisor::defaultPage (xgi::Input* in, xgi::Output* out)
{
	...
	if (configName == "tableEditor")
		*out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame src='/WebPath/html/ConfigurationTableEditor.html?urn=" << this->getApplicationDescriptor ()->getLocalId () << "'></frameset></html>";
	if (configName == "iterate")
		*out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame src='/WebPath/html/Iterate.html?urn=" << this->getApplicationDescriptor ()->getLocalId () << "'></frameset></html>";
	else
		*out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame src='/WebPath/html/ConfigurationGUI.html?urn=" << this->getApplicationDescriptor ()->getLocalId () << "'></frameset></html>";
}
...
//========================================================================================================================
void ConfigurationGUISupervisor::request (const std::string& requestType, cgicc::Cgicc& cgiIn, HttpXmlDocument& xmlOut, const WebUsers::RequestUserInfo& userInfo) try
{
	//Commands
	...
	if (requestType == "saveConfigurationInfo")
	{
		...
		if (!allSupervisorInfo_.isWizardMode ())
		{
			...
			handleSaveConfigurationInfoXML (xmlOut, cfgMgr, configName, columnCSV, tableDescription, columnChoicesCSV, allowOverwrite == "1");
	}
	...
	else if (requestType == "setGroupAliasInActiveBackbone")
	{
		...
		handleSetGroupAliasInBackboneXML (xmlOut, cfgMgr, groupAlias, groupName, ConfigurationGroupKey (groupKey), userInfo.username_);
	}
	else if (requestType == "setVersionAliasInActiveBackbone")
	{
		...
		handleSetVersionAliasInBackboneXML (xmlOut, cfgMgr, versionAlias, configName, ConfigurationVersion (version), userInfo.username_);
	}
	else if (requestType == "setAliasOfGroupMembers")
	{
		...
		handleAliasGroupMembersInBackboneXML (xmlOut, cfgMgr, versionAlias, groupName, ConfigurationGroupKey (groupKey), userInfo.username_);
	}
	...
	else if (requestType == "saveNewConfigurationGroup")
	{
		...
		handleCreateConfigurationGroupXML (xmlOut, cfgMgr, groupName, configList, allowDuplicates, ignoreWarnings, comment, lookForEquivalent);
	}
		...
		if (allCfgInfo.find (configName) != allCfgInfo.end ())
		{
			...
			else if (versionStr.find (ConfigurationManager::ALIAS_VERSION_PREAMBLE) == 0)
			{
				        ...
				        versionAliases[configName].end ())
				{
					...
					__SUP_COUT_WARN__ << "version alias '" << versionStr.substr (ConfigurationManager::ALIAS_VERSION_PREAMBLE.size ()) << "'was not found in active version aliases!" << std::endl;
			}
	...
	else if (requestType == "saveSpecificConfiguration")
	{
		...
		handleCreateConfigurationXML (xmlOut, cfgMgr, configName, ConfigurationVersion (version), temporary, data, dataOffset, userInfo.username_, comment, sourceTableAsIs, lookForEquivalent);
	}
	...
	else if (requestType == "getTreeView")
	{
		...
		handleFillTreeViewXML (xmlOut, cfgMgr, configGroup, ConfigurationGroupKey (configGroupKey), startPath, depth, hideStatusFalse, modifiedTables, filterList);
	}
	else if (requestType == "getTreeNodeCommonFields")
	{
		...
		handleFillTreeNodeCommonFieldsXML (xmlOut, cfgMgr, configGroup, ConfigurationGroupKey (configGroupKey), startPath, depth, modifiedTables, recordList, fieldList);
	}
	else if (requestType == "getUniqueFieldValuesForRecords")
	{
		...
		handleFillUniqueFieldValuesForRecordsXML (xmlOut, cfgMgr, configGroup, ConfigurationGroupKey (configGroupKey), startPath, modifiedTables, recordList, fieldList);
	}
	else if (requestType == "getTreeNodeFieldValues")
	{
		...
		handleFillGetTreeNodeFieldValuesXML (xmlOut, cfgMgr, configGroup, ConfigurationGroupKey (configGroupKey), startPath, modifiedTables, recordList, fieldList);
	}
	else if (requestType == "setTreeNodeFieldValues")
	{
		...
		handleFillSetTreeNodeFieldValuesXML (xmlOut, cfgMgr, configGroup, ConfigurationGroupKey (configGroupKey), startPath, modifiedTables, recordList, fieldList, valueList, userInfo.username_);
	}
	else if (requestType == "addTreeNodeRecords")
	{
		...
		handleFillCreateTreeNodeRecordsXML (xmlOut, cfgMgr, configGroup, ConfigurationGroupKey (configGroupKey), startPath, modifiedTables, recordList, userInfo.username_);
	}
	else if (requestType == "deleteTreeNodeRecords")
	{
		...
		handleFillDeleteTreeNodeRecordsXML (xmlOut, cfgMgr, configGroup, ConfigurationGroupKey (configGroupKey), startPath, modifiedTables, recordList);
	}
	...
	else if (requestType == "saveTreeNodeEdit")
	{
		...
		handleSaveTreeNodeEditXML (xmlOut, cfgMgr, targetTable, ConfigurationVersion (targetTableVersion), editNodeType, CgiDataUtilities::decodeURIComponent (targetUID), CgiDataUtilities::decodeURIComponent (targetColumn), newValue, userInfo.username_);
	}
	else if (requestType == "getLinkToChoices")
	{
		...
		handleGetLinkToChoicesXML (xmlOut, cfgMgr, linkToTableName, ConfigurationVersion (linkToTableVersion), linkIdType, linkIndex, linkInitId);
	}
	else if (requestType == "activateConfigGroup")
	{
		...
		catch (std::runtime_error& e)
		{
			...
			__SUP_COUT__ << "Error detected!\n\n " << e.what () << std::endl;
			xmlOut.addTextElementToData ("Error", "Error activating config group '" + groupName + "(" + groupKey + ")" + ".' Please see details below:\n\n" + std::string (e.what ()));
			__SUP_COUT_ERR__ << "Errors detected so de-activating group: " << groupName << " (" << groupKey << ")" << std::endl;
		...
		if (accumulatedTreeErrors != "")
			xmlOut.addTextElementToData ("Error", "Warnings were found when activating group '" + groupName + "(" + groupKey + ")" + "'! Please see details below:\n\n" + accumulatedTreeErrors);
	}
	...
	else if (requestType == "copyViewToCurrentColumns")
	{
		...
		{
			__SUP_COUT__ << "Error detected!\n\n " << e.what () << std::endl;
			xmlOut.addTextElementToData ("Error", "Error copying view from '" + configName + "_v" + sourceVersion + "'! " + std::string (e.what ()));
		}
		catch (...)
		{
			__SUP_COUT__ << "Error detected!\n\n " << std::endl;
	...
	else if (requestType == "getLastConfigGroups")
	{
		XDAQ_CONST_CALL xdaq::ApplicationDescriptor* gatewaySupervisor =
		    allSupervisorInfo_.isWizardMode () ? allSupervisorInfo_.getWizardDescriptor () : allSupervisorInfo_.getGatewayDescriptor ();
	...
	else if (requestType == "savePlanCommandSequence")
	{
		...
		handleSavePlanCommandSequenceXML (xmlOut, cfgMgr, groupName, ConfigurationGroupKey (groupKey), modifiedTables, userInfo.username_, planName, commands);
	}
	else if (requestType == "mergeGroups")
	{
		...
		handleMergeGroupsXML (xmlOut, cfgMgr, groupANameContext, ConfigurationGroupKey (groupAKeyContext), groupBNameContext, ConfigurationGroupKey (groupBKeyContext), groupANameConfig, ConfigurationGroupKey (groupAKeyConfig), groupBNameConfig, ConfigurationGroupKey (groupBKeyConfig), userInfo.username_, mergeApproach);
	}
	else
	{
                                                             ...
                                                             const std::string&           modifiedTables) try
{
	...
	catch (const std::runtime_error& e)
	{
		...
		{
			__SUP_SS__ << "Failed to determine type of configuration group for " << rootGroupName << "(" << rootGroupKey << ")! " << e.what () << std::endl;
			__SUP_COUT_ERR__ << "\n"
			                 << ss.str ();
			__SS_THROW__;
		}
	...
	catch (...)
	{
		...
		if (rootGroupName.size ())
		{
			__SUP_COUT_ERR__ << "Failed to determine type of configuration group for " << rootGroupName << "(" << rootGroupKey << ")!" << std::endl;
			throw;
		}
	...
	for (auto group : consideredGroups)
	{
		...
		__SUP_COUT__ << "Considering " << group.first << " group " << group.second.first << " (" << group.second.second << ")" << std::endl;

		affected = false;

		std::map<std::string /*name*/, ConfigurationVersion /*version*/> memberMap;
		cfgMgr->loadConfigurationGroup (group.second.first, group.second.second, 0, &memberMap, 0, 0, &groupComment, 0, 0,  //mostly defaults
		                                true /*doNotLoadMember*/);
		...
		for (auto& table : memberMap)
		{
			...
			{
				__SUP_COUT__ << "Affected by " << (*modifiedTablesMapIt).first << ":" << (*modifiedTablesMapIt).second << std::endl;
				affected               = true;
    ...
    std::string*                                                      accumulatedErrors) try
{
	...
	else
	{
		...
		cfgMgr->loadConfigurationGroup (groupName, groupKey, false /*doActivate*/, returnMemberMap, 0 /*progressBar*/, accumulatedErrors, doGetGroupInfo ? &groupComment : 0, doGetGroupInfo ? &groupAuthor : 0, doGetGroupInfo ? &configGroupCreationTime : 0);
	...
	for (auto& activePair : allActivePairs)
	{
		...
		{
			__SUP_COUT__ << "Found modified table " << (*modifiedTablesMapIt).first << ": trying... " << (*modifiedTablesMapIt).second << std::endl;
			...
			{
				__SUP_SS__ << "Modified table version v" << (*modifiedTablesMapIt).second << " failed. Reverting to v" << allCfgInfo.at (activePair.first).configurationPtr_->getView ().getVersion () << "." << std::endl;
	...
	try
	{
		...
		//extract field list
		{
			...
			for (const auto& field : fieldPaths)
				__SUP_COUT__ << "fieldPath " << field << std::endl;
		}
		...
		//extract value list
		{
			...
			for (const auto& value : fieldValues)
				__SUP_COUT__ << "fieldValue " << value << std::endl;
		}
			...
			while (getline (f, recordUID, ','))  //for each record
			{
				...
				for (i = 0; i < fieldPaths.size (); ++i)
				{
					         ...
					         .isTemporaryVersion ())
					{
						...
						__SUP_COUT__ << "Created temporary version " << config->getConfigurationName () << "-v" << temporaryVersion << std::endl;
					}
					//else //else table is already temporary version
					__SUP_COUT__ << "Using temporary version " << config->getConfigurationName () << "-v" << temporaryVersion << std::endl;
...
//
void ConfigurationGUISupervisor::handleFillGetTreeNodeFieldValuesXML (HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupName, const ConfigurationGroupKey& groupKey, const std::string& startPath, const std::string& modifiedTables, const std::string& recordList, const std::string& fieldList)
{
	//	setup active tables based on input group and modified tables
	setupActiveTablesXML (
	    xmlOut,
...

		{
			...
			if (fieldList != "")
			{
				//extract field filter list
				{
					...
					for (auto& field : fieldAcceptList)
						__SUP_COUT__ << "fieldAcceptList " << field << std::endl;
					for (auto& field : fieldRejectList)
						__SUP_COUT__ << "fieldRejectList " << field << std::endl;
				}
			}
			...
			if (recordList == "*")  //handle all records case
			{
				...
				for (auto& record : records)
					__SUP_COUT__ << "recordList " << record << std::endl;
			}
			else if (recordList != "")
			{
				//extract record list
...
//
void ConfigurationGUISupervisor::handleFillTreeViewXML (HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupName, const ConfigurationGroupKey& groupKey, const std::string& startPath, unsigned int depth, bool hideStatusFalse, const std::string& modifiedTables, const std::string& filterList)
{
	//return xml
	//	<groupName="groupName"/>
	//	<tree="path">
	...
	try
	{
		...
		else
		{
			...
			StringMacros::getMapFromString (filterList, filterMap, std::set<char> ({';'}) /*pair delimiters*/, std::set<char> ({'='}) /*name/value delimiters*/);
...
void ConfigurationGUISupervisor::recursiveTreeToXML (const ConfigurationTree& t, unsigned int depth, HttpXmlDocument& xmlOut, DOMElement* parentEl, bool hideStatusFalse)
{
                                                            ...
                                                            const std::string&          linkInitId) try
{
	...
	{
		__SUP_SS__ << "Target table version (" << version << ") is not the currently active version (" << config->getViewVersion ()
...
{
	__SUP_COUT__ << "Merging context group pair " << groupANameContext << " (" << groupAKeyContext << ") & " << groupBNameContext << " (" << groupBKeyContext << ") and config group pair " << groupANameConfig << " (" << groupAKeyConfig << ") & " << groupBNameConfig << " (" << groupBKeyConfig << ") with approach '" << mergeApproach << __E__;
	...
	{
		cfgMgr->loadConfigurationGroup (groupANameContext, groupAKeyContext, false /*doActivate*/, &memberMapAContext, 0 /*progressBar*/, 0 /*accumulateErrors*/, 0 /*groupComment*/, 0 /*groupAuthor*/, 0 /*groupCreationTime*/, false /*doNotLoadMember*/, 0 /*groupTypeString*/
		);
		__SUP_COUTV__ (StringMacros::mapToString (memberMapAContext));

		cfgMgr->loadConfigurationGroup (groupBNameContext, groupBKeyContext, false /*doActivate*/, &memberMapBContext, 0 /*progressBar*/, 0 /*accumulateErrors*/, 0 /*groupComment*/, 0 /*groupAuthor*/, 0 /*groupCreationTime*/, false /*doNotLoadMember*/, 0 /*groupTypeString*/
		);
	...
	{
		cfgMgr->loadConfigurationGroup (groupANameConfig, groupAKeyConfig, false /*doActivate*/, &memberMapAConfig, 0 /*progressBar*/, 0 /*accumulateErrors*/, 0 /*groupComment*/, 0 /*groupAuthor*/, 0 /*groupCreationTime*/, false /*doNotLoadMember*/, 0 /*groupTypeString*/
		);
		__SUP_COUTV__ (StringMacros::mapToString (memberMapAConfig));

		cfgMgr->loadConfigurationGroup (groupBNameConfig, groupBKeyConfig, false /*doActivate*/, &memberMapBConfig, 0 /*progressBar*/, 0 /*accumulateErrors*/, 0 /*groupComment*/, 0 /*groupAuthor*/, 0 /*groupCreationTime*/, false /*doNotLoadMember*/, 0 /*groupTypeString*/
		);
				...
				else if (memberMapAref[bkey.first] != bkey.second)
				{
					...
					if (i == 1)
					{
						...
						try
						{
							...
							newVersion = saveModifiedVersionXML (xmlOut, cfgMgr, bkey.first, ConfigurationVersion () /*original source version*/, false /* makeTemporary */, config, newVersion /*temporary modified version*/, false /*ignore duplicates*/, true /*look for equivalent*/);
						}
						catch (std::runtime_error& e)
						{
							__SUP_SS__ << "There was an error saving the '" << config->getConfigurationName () << "' merge result to a persistent table version. "
							           << "Perhaps you can modify this table in one of the groups to resolve this issue, and then re-merge." << __E__ << e.what ();
							__SS_THROW__;
						}
	...
	if (!skippingContextPair)
	{
		...
		ConfigurationGroupKey newKeyContext =
		    cfgMgr->saveNewConfigurationGroup (groupANameContext, memberMapAContext, "Merger of group " + groupANameContext + " (" + groupAKeyContext.toString () + ") and " + groupBNameContext + " (" + groupBKeyContext.toString () + ").");
	...
	if (!skippingConfigPair)
	{
		...
		ConfigurationGroupKey newKeyConfig =
		    cfgMgr->saveNewConfigurationGroup (groupANameConfig, memberMapAConfig, "Merger of group " + groupANameConfig + " (" + groupAKeyConfig.toString () + ") and " + groupBNameConfig + " (" + groupBKeyConfig.toString () + ").");
...
{
	__SUP_SS__ << "Error merging context group pair " << groupANameContext << " (" << groupAKeyContext << ") & " << groupBNameContext << " (" << groupBKeyContext << ") and config group pair " << groupANameConfig << " (" << groupAKeyConfig << ") & " << groupBNameConfig << " (" << groupBKeyConfig << ") with approach '" << mergeApproach << "': \n\n"
	           << e.what () << std::endl;
	__SUP_COUT_ERR__ << "\n"
	                 << ss.str () << std::endl;
	xmlOut.addTextElementToData ("Error", ss.str ());
...
{
	__SUP_SS__ << "Unknown error merging context group pair " << groupANameContext << " (" << groupAKeyContext << ") & " << groupBNameContext << " (" << groupBKeyContext << ") and config group pair " << groupANameConfig << " (" << groupAKeyConfig << ") & " << groupBNameConfig << " (" << groupBKeyConfig << ") with approach '" << mergeApproach << ".' \n\n";
	__SUP_COUT_ERR__ << "\n"
	                 << ss.str () << std::endl;
	xmlOut.addTextElementToData ("Error", ss.str ());
}
                                                                   ...
                                                                   const std::string&           commandString) try
{
	...
	catch (...)
	{
		...
		{
			__SUP_COUT__ << "Erasing temporary version " << planTable.configName_ << "-v" << planTable.temporaryVersion_ << std::endl;
		...
		{
			__SUP_COUT__ << "Erasing temporary version " << targetTable.configName_ << "-v" << targetTable.temporaryVersion_ << std::endl;
			//erase with proper version management
		...
		for (auto& modifiedConfig : commandTypeToCommandTableMap)
		{
			...
			{
				__SUP_COUT__ << "Erasing temporary version " << modifiedConfig.second.configName_ << "-v" << modifiedConfig.second.temporaryVersion_ << std::endl;
				//erase with proper version management
	...
	ConfigurationVersion finalVersion = saveModifiedVersionXML (xmlOut, cfgMgr, planTable.configName_, planTable.originalVersion_, true /*make temporary*/, planTable.config_, planTable.temporaryVersion_, true /*ignoreDuplicates*/);  //save temporary version properly

	__SUP_COUT__ << "Final plan version is " << planTable.configName_ << "-v" << finalVersion << std::endl;

	finalVersion = saveModifiedVersionXML (xmlOut, cfgMgr, targetTable.configName_, targetTable.originalVersion_, true /*make temporary*/, targetTable.config_, targetTable.temporaryVersion_, true /*ignoreDuplicates*/);  //save temporary version properly
	...
	for (auto& modifiedConfig : commandTypeToCommandTableMap)
	{
		if (!modifiedConfig.second.modified_)
		{
			if (modifiedConfig.second.createdTemporaryVersion_)  //if temporary version created here
			{
				__SUP_COUT__ << "Erasing unmodified temporary version " << modifiedConfig.second.configName_ << "-v" << modifiedConfig.second.temporaryVersion_ << std::endl;
				//erase with proper version management
				cfgMgr->eraseTemporaryVersion (modifiedConfig.second.configName_,
				                               modifiedConfig.second.temporaryVersion_);
			}
		...
		finalVersion = saveModifiedVersionXML (xmlOut, cfgMgr, modifiedConfig.second.configName_, modifiedConfig.second.originalVersion_, true /*make temporary*/, modifiedConfig.second.config_, modifiedConfig.second.temporaryVersion_, true /*ignoreDuplicates*/);  //save temporary version properly
                                                            ...
                                                            const std::string&      author) try
{
	    ...
	    type == "link-comment")
	{
		...
		{
			__SUP_SS__ << "Comment '" << newValue << "' is the same as the current comment. No need to save change." << std::endl;
			__SS_THROW__;
		}
	}
	...
	try
	{
		         ...
		         type == "value-bitmap")
		{
			...
			if (!cfgView->setURIEncodedValue (newValue, row, col, author))
			{
				...
				__SUP_SS__ << "Value '" << newValue << "' is the same as the current value. No need to save change to tree node." << std::endl;
				__SS_THROW__;
			}
		}
		else if (type == "link-UID" || type == "link-GroupID")
		{
			...
			if (type == "link-GroupID")
			{
				...
				else  //if changes, save it
				{
					try
					{
						...
						saveModifiedVersionXML (xmlOut, cfgMgr, configName, version, true /*make temporary*/, config, temporaryVersion, true /*ignoreDuplicates*/);  //save temporary version properly
					}
				...
				else  //if changes, save it
				{
					try
					{
						...
						saveModifiedVersionXML (xmlOut, cfgMgr, newTable, version, true /*make temporary*/, config, temporaryVersion, true /*ignoreDuplicates*/);  //save temporary version properly
					}
					catch (std::runtime_error& e)  //erase temporary view before re-throwing error
					{
				...
				{
					__SUP_SS__ << "Link to table '" << newTable << "', linkID '" << newLinkId << "', and selected group members are the same as the current value. "
			...
			{                        //RAR: block also becuase versions are temporary at this point anyway, might as well abuse temporary versions
				__SUP_SS__ << "Link to table '" << newTable << "' and linkID '" << newLinkId << "' are the same as the current values. No need to save change to tree node." << std::endl;
				__SS_THROW__;
			}
		}
                                                                 ...
                                                                 ConfigurationGroupKey   groupKey) try
{
	...
	try
	{
		...
		cfgMgr->loadConfigurationGroup (groupName, groupKey, false /*doActivate*/, &memberMap, 0 /*progressBar*/, &accumulateErrors /*accumulateErrors*/, &groupComment, &groupAuthor, &groupCreationTime, false /*doNotLoadMember*/, &groupTypeString, &groupMemberAliases);
                                                            ...
                                                            bool                    allowIllegalColumns) try
{
	...
	//existing table versions
	{
		...
		std::map<std::string /*table name*/, std::map<std::string /*version alias*/, ConfigurationVersion /*aliased version*/>> versionAliases;
		try
		{
	...
	else  //use view version
	{
		...
		{
			__SUP_SS__ << "Failed to get table " << tableName << " version " << version << "... defaulting to mock-up! " << std::endl;
	...
	{
		__SUP_SS__ << "\n\nThere were warnings found when loading the table " << tableName << ":v" << version << ". Please see the details below:\n\n"
		           << "The source column size was found to be " << cfgViewPtr->getDataColumnSize () << ", and the current number of columns for this table is " << cfgViewPtr->getNumberOfColumns () << ". This resulted in a count of " << cfgViewPtr->getSourceColumnMismatch () << " source column mismatches, and a count of " << cfgViewPtr->getSourceColumnMissing () << " table entries missing in " << cfgViewPtr->getNumberOfRows () << " row(s) of data." << std::endl;
                                                                         ...
                                                                         bool                    lookForEquivalent)
{
	...
	if (!ignoreDuplicates)
	{
...

		{
			...
			__SUP_COUT__ << "Filling up cached from " << config->getNumberOfStoredViews () << " to max count of " << config->MAX_VIEWS_IN_CACHE << std::endl;
		...
		duplicateVersion = config->checkForDuplicate (temporaryModifiedVersion,
		                                              (!originalVersion.isTemporaryVersion () && !makeTemporary) ? ConfigurationVersion () :  //if from persistent to persistent, then include original version in search
		                                                  originalVersion);
		...
		{
			__SUP_SS__ << "This version is identical to another version currently cached v" << duplicateVersion << ". No reason to save a duplicate." << std::endl;
			__SUP_COUT_ERR__ << "\n"
			                 << ss.str ();
                                                               ...
                                                               bool                    lookForEquivalent) try
{
	...
	if (!version.isInvalid ())  //if not using mock-up, then the starting version is the active one
	{
		    ...
		    config->getViewP ()->getSourceColumnMismatch () != 0)
		{
			...
			__SUP_COUT_INFO__ << "Source view v" << version << " has a mismatch in the number of columns, so using mockup as source." << std::endl;
	...
	saveModifiedVersionXML (xmlOut, cfgMgr, configName, version, makeTemporary, config, temporaryVersion, false /*ignoreDuplicates*/, lookForEquivalent || sourceTableAsIs /*lookForEquivalent*/);
}
catch (std::runtime_error& e)
{
	__SUP_COUT__ << "Error detected!\n\n " << e.what () << std::endl;
    ...
    bool                    lookForEquivalent) try
{
	...
	while (c < configList.length ())
	{
		...
		if (versionStr.find (ConfigurationManager::ALIAS_VERSION_PREAMBLE) == 0)
		{
			...
			{
				__SUP_SS__ << "version alias '" << versionStr.substr (ConfigurationManager::ALIAS_VERSION_PREAMBLE.size ()) << "' was not found in active version aliases!" << std::endl;
				__SUP_COUT_ERR__ << "\n"
				                 << ss.str ();
		...
		if (version.isMockupVersion ())
		{
			...
			__SUP_COUT__ << "Creating version from mock-up for name: " << name << " inputVersionStr: " << versionStr << std::endl;

			//set table comment
			config->getTemporaryView (temporaryVersion)->setComment ("Auto-generated from mock-up.");

			//finish off the version creation
			version = saveModifiedVersionXML (xmlOut, cfgMgr, name, ConfigurationVersion () /*original source is mockup*/, false /* makeTemporary */, config, temporaryVersion /*temporary modified version*/, false /*ignore duplicates*/, true /*look for equivalent*/);
	...
	try
	{
		...
		for (auto& groupMemberPair : groupMembers)
		{
			...
			if (cfgViewPtr->getDataColumnSize () !=
			    ...
			    cfgViewPtr->getSourceColumnMismatch () != 0)  //check for column size mismatch
			{
				__SUP_SS__ << "\n\nThere were errors found in loading a member table " << groupMemberPair.first << ":v" << cfgViewPtr->getVersion () << ". Please see the details below:\n\n"
				           << "The source column size was found to be " << cfgViewPtr->getDataColumnSize () << ", and the current number of columns for this table is " << cfgViewPtr->getNumberOfColumns () << ". This resulted in a count of " << cfgViewPtr->getSourceColumnMismatch () << " source column mismatches, and a count of " << cfgViewPtr->getSourceColumnMissing () << " table entries missing in " << cfgViewPtr->getNumberOfRows () << " row(s) of data." << std::endl;
                                                                 ...
                                                                 bool                    allowOverwrite)
{
	...
	outss << "\t\t\t<VIEW Name=\"" << capsName << "\" Type=\"File,Database,DatabaseTest\" Description=\"" << tableDescription << "\">\n";
	...
	while (k != (int)(std::string::npos))
	{
		...
		{  //error! non-alpha
			xmlOut.addTextElementToData ("Error", std::string ("For column name '") + data.substr (i, j - i) + "' - " + e.what ());
			return;
		}
	...
	if (!fp)
	{
		xmlOut.addTextElementToData ("Error", "Failed to open destination Configuration Info file:" + (CONFIG_INFO_PATH + configName + CONFIG_INFO_EXT));
		return;
	}
                                                                   ...
                                                                   const std::string&      author) try
{
	...
	if (isDifferent)  //make new version if different
	{
		...
		newAssignedVersion = saveModifiedVersionXML (xmlOut, cfgMgr, config->getConfigurationName (), originalVersion, false /*makeTemporary*/, config, temporaryVersion, false /*ignoreDuplicates*/, true /*lookForEquivalent*/);
	}
                                                                       ...
                                                                       const std::string&      author) try
{
	...
	{
		xmlOut.addTextElementToData ("Error", "Configuration group \"" + ConfigurationGroupKey::getFullGroupString (groupName, groupKey) + "\" can not be retrieved!");
		return;
	}
                                                        ...
                                                        ConfigurationManagerRW* cfgMgr)
{
	...
	__SUP_COUT__ << "activeVersions[\"" << groupAliasesTableName << "\"]=" << activeVersions[groupAliasesTableName] << std::endl;
	...
	for (auto& aliasNodePair : aliasNodePairs)
	{
		...
		try
		{
			cfgMgr->loadConfigurationGroup (groupName, ConfigurationGroupKey (groupKey), 0, 0, 0, 0, &groupComment, 0, 0,  //mostly defaults
			                                true /*doNotLoadMembers*/,
			                                &groupType);
		}
		catch (...)
		{
			__SUP_COUT_WARN__ << "Failed to load group '" << groupName << "(" << groupKey << ")' to extract group comment and type." << std::endl;
		}
                                                          ...
                                                          ConfigurationManagerRW* cfgMgr)
{
	...
	__SUP_COUT__ << "activeVersions[\"" << versionAliasesTableName << "\"]=" << activeVersions[versionAliasesTableName] << std::endl;
		...
		for (auto& keyInSet : groupInfo.second.keys_)
		{
			...
			if (returnMembers)
			{
				...
				if (loadingHistoricalInfo)
				{
					...
					{
						groupTypeString = "Invalid";
						__SUP_COUT_WARN__ << "Failed to load group '" << groupName << "(" << keyInSet << ")' to extract group comment and type." << std::endl;
					}
                                                          ...
                                                          bool                    allowIllegalColumns)
{
	...
	if (accumulatedErrors != "")
		xmlOut.addTextElementToData ("Error", std::string ("Column errors were allowed for this request, ") + "but please note the following errors:\n" + accumulatedErrors);
}
