...
ConfigurationGUISupervisor::ConfigurationGUISupervisor(xdaq::ApplicationStub* stub) : CoreSupervisorBase(stub) {
...
void ConfigurationGUISupervisor::destroy(void) {
  ...
  for (std::map<std::string, ConfigurationManagerRW*>::iterator it = userConfigurationManagers_.begin();
       it != userConfigurationManagers_.end(); ++it) {
    delete it->second;
    it->second = 0;
  }
  ...
  if (ConfigurationInterface::getInstance(true) != 0) delete ConfigurationInterface::getInstance(true);
}
...
void ConfigurationGUISupervisor::setSupervisorPropertyDefaults(void) {
  CorePropertySupervisorBase::setSupervisorProperty(
      ...
      "*=10 | deleteTreeNodeRecords=255 | saveConfigurationInfo=255 | deleteConfigurationInfo=255");  // experienced
                                                                                                      // users to edit,
                                                                                                      // admins to delete
  CorePropertySupervisorBase::setSupervisorProperty(
...
//========================================================================================================================
void ConfigurationGUISupervisor::request(const std::string& requestType, cgicc::Cgicc& cgiIn, HttpXmlDocument& xmlOut,
  ...
  ConfigurationManagerRW* cfgMgr =
      refreshUserSession(userInfo.username_, userInfo.activeUserSessionIndex_, (refresh == "1"));
  ...
  if (requestType == "saveConfigurationInfo") {
    ...
    if (!allSupervisorInfo_.isWizardMode()) {
      ...
      handleSaveConfigurationInfoXML(xmlOut, cfgMgr, configName, columnCSV, tableDescription, columnChoicesCSV,
                                     allowOverwrite == "1");
  ...
  //	}
    ...
    std::map<std::pair<std::string, std::string>, std::string> allDefaults = ViewColumnInfo::getAllDefaultsForGUI();

    for (const auto& type : allTypes) xmlOut.addTextElementToData("columnTypeForGUI", type);
    for (const auto& dataType : allDataTypes) xmlOut.addTextElementToData("columnDataTypeForGUI", dataType);

    for (const auto& colDefault : allDefaults) {
      xmlOut.addTextElementToData("columnDefaultDataType", colDefault.first.first);
      xmlOut.addTextElementToData("columnDefaultTypeFilter", colDefault.first.second);
    ...
    handleSetGroupAliasInBackboneXML(xmlOut, cfgMgr, groupAlias, groupName, ConfigurationGroupKey(groupKey),
                                     userInfo.username_);
    ...
    handleSetVersionAliasInBackboneXML(xmlOut, cfgMgr, versionAlias, configName, ConfigurationVersion(version),
                                       userInfo.username_);
    ...
    handleAliasGroupMembersInBackboneXML(xmlOut, cfgMgr, versionAlias, groupName, ConfigurationGroupKey(groupKey),
                                         userInfo.username_);
    ...
    bool doNotReturnMembers =
        CgiDataUtilities::getDataAsInt(cgiIn, "doNotReturnMembers") == 1 ? true : false;  // from GET
    ...
    for (auto& member : members) xmlOut.addTextElementToData("ContextMember", member);
  } else if (requestType == "getBackboneMemberNames") {
    std::set<std::string> members = cfgMgr->getBackboneMemberNames();

    for (auto& member : members) xmlOut.addTextElementToData("BackboneMember", member);
  } else if (requestType == "getIterateMemberNames") {
    std::set<std::string> members = cfgMgr->getIterateMemberNames();

    for (auto& member : members) xmlOut.addTextElementToData("IterateMember", member);
    ...
    handleCreateConfigurationGroupXML(xmlOut, cfgMgr, groupName, configList, allowDuplicates, ignoreWarnings, comment,
                                      lookForEquivalent);
    ...
    if (allCfgInfo.find(configName) != allCfgInfo.end()) {
      if (versionStr == "" &&  // take latest version if no version specified
        ...
        //			if(versionAlias == ConfigurationManager::SCRATCH_VERSION_ALIAS) //NOT NEEDED IF SCRATCH IS
        //ALWAYS ALIAS
        //			{
        ...
        if (versionAliases.find(configName) != versionAliases.end() &&
            versionAliases[configName].find(versionStr.substr(ConfigurationManager::ALIAS_VERSION_PREAMBLE.size())) !=
                versionAliases[configName].end()) {
          version = versionAliases[configName][versionStr.substr(ConfigurationManager::ALIAS_VERSION_PREAMBLE.size())];
          __SUP_COUT__ << "version alias translated to: " << version << std::endl;
        } else
          __SUP_COUT_WARN__ << "version alias '"
    ...
    handleGetConfigurationXML(xmlOut, cfgMgr, configName, ConfigurationVersion(version), (allowIllegalColumns == "1"));
    ...
    __SUP_COUT__ << "configName: " << configName << " version: " << version << " temporary: " << temporary
    ...
    handleCreateConfigurationXML(xmlOut, cfgMgr, configName, ConfigurationVersion(version), temporary, data, dataOffset,
                                 userInfo.username_, comment, sourceTableAsIs, lookForEquivalent);
    ...
    handleFillTreeViewXML(xmlOut, cfgMgr, configGroup, ConfigurationGroupKey(configGroupKey), startPath, depth,
                          hideStatusFalse, modifiedTables, filterList);
    ...
    handleFillTreeNodeCommonFieldsXML(xmlOut, cfgMgr, configGroup, ConfigurationGroupKey(configGroupKey), startPath,
                                      depth, modifiedTables, recordList, fieldList);
    ...
    handleFillGetTreeNodeFieldValuesXML(xmlOut, cfgMgr, configGroup, ConfigurationGroupKey(configGroupKey), startPath,
                                        modifiedTables, recordList, fieldList);
    ...
    handleFillSetTreeNodeFieldValuesXML(xmlOut, cfgMgr, configGroup, ConfigurationGroupKey(configGroupKey), startPath,
                                        modifiedTables, recordList, fieldList, valueList, userInfo.username_);
    ...
    handleFillCreateTreeNodeRecordsXML(xmlOut, cfgMgr, configGroup, ConfigurationGroupKey(configGroupKey), startPath,
                                       modifiedTables, recordList, userInfo.username_);
    ...
    handleFillDeleteTreeNodeRecordsXML(xmlOut, cfgMgr, configGroup, ConfigurationGroupKey(configGroupKey), startPath,
                                       modifiedTables, recordList);
    ...
    handleGetAffectedGroupsXML(xmlOut, cfgMgr, groupName, ConfigurationGroupKey(groupKey), modifiedTables);
    ...
    handleSaveTreeNodeEditXML(xmlOut, cfgMgr, targetTable, ConfigurationVersion(targetTableVersion), editNodeType,
                              CgiDataUtilities::decodeURIComponent(targetUID),
                              CgiDataUtilities::decodeURIComponent(targetColumn), newValue, userInfo.username_);
    ...
    handleGetLinkToChoicesXML(xmlOut, cfgMgr, linkToTableName, ConfigurationVersion(linkToTableVersion), linkIdType,
                              linkIndex, linkInitId);
  } else if (requestType == "activateConfigGroup") {
    std::string groupName = CgiDataUtilities::getData(cgiIn, "groupName");
    std::string groupKey = CgiDataUtilities::getData(cgiIn, "groupKey");
    bool ignoreWarnings = CgiDataUtilities::getDataAsInt(cgiIn, "ignoreWarnings");

    __SUP_COUT__ << "Activating config: " << groupName << "(" << groupKey << ")" << std::endl;
    __SUP_COUT__ << "ignoreWarnings: " << ignoreWarnings << std::endl;
    ...
    try {
      cfgMgr->activateConfigurationGroup(
          groupName, ConfigurationGroupKey(groupKey),
      ...
      __SUP_COUT__ << "Error detected!\n\n " << e.what() << std::endl;
      xmlOut.addTextElementToData("Error", "Error activating config group '" + groupName + "(" + groupKey + ")" +
                                               ".' Please see details below:\n\n" + std::string(e.what()));
      __SUP_COUT_ERR__ << "Errors detected so de-activating group: " << groupName << " (" << groupKey << ")"
                       << std::endl;
    ...
    if (accumulatedTreeErrors != "")
      xmlOut.addTextElementToData("Error", "Warnings were found when activating group '" + groupName + "(" + groupKey +
    ...
    try {
      ...
      newTemporaryVersion = cfgMgr->copyViewToCurrentColumns(configName, ConfigurationVersion(sourceVersion));
      //
      ...
      xmlOut.addTextElementToData(
          "Error", "Error copying view from '" + configName + "_v" + sourceVersion + "'! " + std::string(e.what()));
    } catch (...) {
      __SUP_COUT__ << "Error detected!\n\n " << std::endl;
      xmlOut.addTextElementToData("Error", "Error copying view from '" + configName + "_v" + sourceVersion + "'! ");
    }

    handleGetConfigurationXML(xmlOut, cfgMgr, configName, newTemporaryVersion);
  } else if (requestType == "getLastConfigGroups") {
    XDAQ_CONST_CALL xdaq::ApplicationDescriptor* gatewaySupervisor = allSupervisorInfo_.isWizardMode()
    ...
    std::pair<std::string /*group name*/, ConfigurationGroupKey> theGroup =
        theRemoteWebUsers_.getLastConfigGroup(gatewaySupervisor, "Configured", timeString);
    ...
    theGroup = theRemoteWebUsers_.getLastConfigGroup(gatewaySupervisor, "Started", timeString);
    ...
    handleSavePlanCommandSequenceXML(xmlOut, cfgMgr, groupName, ConfigurationGroupKey(groupKey), modifiedTables,
                                     userInfo.username_, planName, commands);
    ...
    handleMergeGroupsXML(xmlOut, cfgMgr, groupANameContext, ConfigurationGroupKey(groupAKeyContext), groupBNameContext,
                         ConfigurationGroupKey(groupBKeyContext), groupANameConfig,
                         ConfigurationGroupKey(groupAKeyConfig), groupBNameConfig,
                         ConfigurationGroupKey(groupBKeyConfig), userInfo.username_, mergeApproach);
  } else {
  ...
  std::map<std::string /*type*/, std::pair<std::string /*groupName*/, ConfigurationGroupKey>> activeGroupMap =
  ...
  for (auto& type : activeGroupMap) {
    ...
    //__SUP_COUT__ << "ActiveGroup " << type.first << " " << type.second.first << "(" << type.second.second << ")" <<
    //__E__;
  }
  ...
  __SS__ << "A fatal error occurred while handling the request '" << requestType << ".' Error: " << e.what() << __E__;
  __COUT_ERR__ << "\n" << ss.str();
  xmlOut.addTextElementToData("Error", ss.str());
  ...
  __SS__ << "A fatal error occurred while handling the request '" << requestType << ".'" << __E__;
  __COUT_ERR__ << "\n" << ss.str();
  xmlOut.addTextElementToData("Error", ss.str());
...
void ConfigurationGUISupervisor::handleGetAffectedGroupsXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
  ...
  if (consideredGroups[ConfigurationManager::ACTIVE_GROUP_NAME_CONTEXT].second.isInvalid()) {
    ...
    if (cfgMgr->getFailedConfigurationGroups().find(ConfigurationManager::ACTIVE_GROUP_NAME_CONTEXT) !=
    ...
    } else if (cfgMgr->getFailedConfigurationGroups().find(ConfigurationManager::ACTIVE_GROUP_NAME_UNKNOWN) !=
               cfgMgr->getFailedConfigurationGroups().end()) {
      consideredGroups[ConfigurationManager::ACTIVE_GROUP_NAME_CONTEXT] =
  ...
  if (consideredGroups[ConfigurationManager::ACTIVE_GROUP_NAME_CONFIGURATION].second.isInvalid()) {
    ...
    if (cfgMgr->getFailedConfigurationGroups().find(ConfigurationManager::ACTIVE_GROUP_NAME_CONFIGURATION) !=
        cfgMgr->getFailedConfigurationGroups().end()) {
  ...
  try {
    ...
    cfgMgr->loadConfigurationGroup(rootGroupName, rootGroupKey, 0, &rootGroupMemberMap, 0, 0, 0, 0, 0,  // defaults
                                   true);  // doNotLoadMember

    const std::string& groupType = cfgMgr->getTypeNameOfGroup(rootGroupMemberMap);

    consideredGroups[groupType] = std::pair<std::string, ConfigurationGroupKey>(rootGroupName, rootGroupKey);
  } catch (const std::runtime_error& e) {
    // if actual group name was attempted re-throw
    if (rootGroupName.size()) {
      __SUP_SS__ << "Failed to determine type of configuration group for " << rootGroupName << "(" << rootGroupKey
                 << ")! " << e.what() << std::endl;
      __SUP_COUT_ERR__ << "\n" << ss.str();
      __SS_THROW__;
    }
    ...
    __SUP_COUT__
        << "Did not modify considered active groups due to empty root group name - assuming this was intentional."
        << std::endl;
  } catch (...) {
    // if actual group name was attempted re-throw
    if (rootGroupName.size()) {
      __SUP_COUT_ERR__ << "Failed to determine type of configuration group for " << rootGroupName << "(" << rootGroupKey
                       << ")!" << std::endl;
      throw;
    }
  ...
  std::map<std::string /*name*/, ConfigurationVersion /*version*/>::iterator modifiedTablesMapIt;
  {
    ...
    while (getline(f, table, ',')) {
      ...
      modifiedTablesMap.insert(
          std::pair<std::string /*name*/, ConfigurationVersion /*version*/>(table, ConfigurationVersion(version)));
    }
    __SUP_COUT__ << modifiedTables << std::endl;
    for (auto& pair : modifiedTablesMap)
      __SUP_COUT__ << "modified table " << pair.first << ":" << pair.second << std::endl;
  }
  ...
  for (auto group : consideredGroups) {
    ...
    __SUP_COUT__ << "Considering " << group.first << " group " << group.second.first << " (" << group.second.second
                 << ")" << std::endl;

    affected = false;

    std::map<std::string /*name*/, ConfigurationVersion /*version*/> memberMap;
    cfgMgr->loadConfigurationGroup(group.second.first, group.second.second, 0, &memberMap, 0, 0, &groupComment, 0,
                                   0,  // mostly defaults
                                   true /*doNotLoadMember*/);
    ...
    for (auto& table : memberMap) {
      if ((modifiedTablesMapIt = modifiedTablesMap.find(table.first)) !=
        ...
        __SUP_COUT__ << "Affected by " << (*modifiedTablesMapIt).first << ":" << (*modifiedTablesMapIt).second
                     << std::endl;
        affected = true;
    ...
    if (affected) {
      ...
      xmlOut.addTextElementToParent("GroupKey", group.second.second.toString(), parentEl);
      ...
      for (auto& table : memberMap) {
        ...
        xmlOut.addTextElementToParent("MemberVersion", table.second.toString(), parentEl);
      }
    }
  }
} catch (std::runtime_error& e) {
...
void ConfigurationGUISupervisor::setupActiveTablesXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupName,
    const ConfigurationGroupKey& groupKey, const std::string& modifiedTables, bool refreshAll, bool doGetGroupInfo,
    std::map<std::string /*name*/, ConfigurationVersion /*version*/>* returnMemberMap, bool outputActiveTables,
  ...
  if (usingActiveGroups || refreshAll) cfgMgr->getAllConfigurationInfo(true, accumulatedErrors);  // do refresh
  ...
  if (usingActiveGroups) {
    ...
    __SUP_COUT__ << "Loading group '" << groupName << "(" << groupKey << ")'" << std::endl;

    std::string groupComment, groupAuthor, configGroupCreationTime;

    // only same member map if object pointer was passed
    cfgMgr->loadConfigurationGroup(groupName, groupKey, false /*doActivate*/, returnMemberMap, 0 /*progressBar*/,
                                   accumulatedErrors, doGetGroupInfo ? &groupComment : 0,
                                   doGetGroupInfo ? &groupAuthor : 0, doGetGroupInfo ? &configGroupCreationTime : 0);
  ...
  for (auto& activePair : allActivePairs) {
    if (outputActiveTables) xmlOut.addTextElementToData("ActiveTableName", activePair.first);

    // check if name is in modifiedTables
    // if so, activate the temporary version
    if ((modifiedTablesMapIt = modifiedTablesMap.find(activePair.first)) != modifiedTablesMap.end()) {
      __SUP_COUT__ << "Found modified table " << (*modifiedTablesMapIt).first << ": trying... "
                   << (*modifiedTablesMapIt).second << std::endl;

      try {
        allCfgInfo.at(activePair.first).configurationPtr_->setActiveView((*modifiedTablesMapIt).second);
      } catch (...) {
        __SUP_SS__ << "Modified table version v" << (*modifiedTablesMapIt).second << " failed. Reverting to v"
...
void ConfigurationGUISupervisor::handleFillCreateTreeNodeRecordsXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupName,
    const ConfigurationGroupKey& groupKey, const std::string& startPath, const std::string& modifiedTables,
    const std::string& recordList, const std::string& author) {
  //	setup active tables based on input group and modified tables
  setupActiveTablesXML(xmlOut, cfgMgr, groupName, groupKey, modifiedTables, true /* refresh all */,
                       false /* getGroupInfo */, 0 /* returnMemberMap */, false /* outputActiveTables */);

  try {
    ConfigurationTree targetNode = cfgMgr->getNode(startPath);
    ConfigurationBase* config = cfgMgr->getConfigurationByName(targetNode.getConfigurationName());
    ...
    // extract record list
    {
      ...
      while (getline(f, recordUID, ','))  // for each record
      {
        ...
        {
          if (!(temporaryVersion = targetNode.getConfigurationVersion()).isTemporaryVersion()) {
            ...
            cfgMgr->saveNewConfiguration(targetNode.getConfigurationName(), temporaryVersion,
...
void ConfigurationGUISupervisor::handleFillModifiedTablesXML(HttpXmlDocument& xmlOut,
                                                             ConfigurationManagerRW* cfgMgr) try {
  ...
  try {
    ...
    // extract record list
    {
      ...
      while (getline(f, recordUID, ','))  // for each record
      {
        ...
        unsigned int row = config->getViewP()->findRow(config->getViewP()->getColUID(), recordUID);
...
void ConfigurationGUISupervisor::handleFillSetTreeNodeFieldValuesXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupName,
    const ConfigurationGroupKey& groupKey, const std::string& startPath, const std::string& modifiedTables,
    const std::string& recordList, const std::string& fieldList, const std::string& valueList,
  ...
  try {
    ...
    // extract field list
    {
      ...
      for (const auto& field : fieldPaths) __SUP_COUT__ << "fieldPath " << field << std::endl;
    }
    ...
    // extract value list
    {
      ...
      if (valueList.size() && valueList[valueList.size() - 1] == ',') fieldValues.push_back("");

      __SUP_COUT__ << valueList << std::endl;
      for (const auto& value : fieldValues) __SUP_COUT__ << "fieldValue " << value << std::endl;
    }
      ...
      while (getline(f, recordUID, ','))  // for each record
      {
        ...
        for (i = 0; i < fieldPaths.size(); ++i) {
          ...
          ConfigurationTree targetNode =
              cfgMgr->getNode(startPath + "/" + recordUID + "/" + fieldPaths[i], true /*doNotThrowOnBrokenUIDLinks*/);
          ...
          __SUP_COUT__ << "Getting table " << targetNode.getFieldConfigurationName() << std::endl;

          // if link must get parent config name
          config = cfgMgr->getConfigurationByName(targetNode.getFieldConfigurationName());  // NOT getConfigurationName!
          if (!(temporaryVersion = config->getViewP()->getVersion()).isTemporaryVersion()) {
            // create temporary version for editing
            temporaryVersion = config->createTemporaryView(config->getViewP()->getVersion());
            cfgMgr->saveNewConfiguration(config->getConfigurationName(), temporaryVersion,
                                         true);  // proper bookkeeping for temporary version with the new version

            __SUP_COUT__ << "Created temporary version " << config->getConfigurationName() << "-v" << temporaryVersion
                         << std::endl;
          }
          // else //else table is already temporary version
          __SUP_COUT__ << "Using temporary version " << config->getConfigurationName() << "-v" << temporaryVersion
                       << std::endl;

          // copy "value" type edit from handleSaveTreeNodeEditXML() functionality
          config->getViewP()->setURIEncodedValue(fieldValues[i], targetNode.getFieldRow(), targetNode.getFieldColumn(),
                                                 author);
...
void ConfigurationGUISupervisor::handleFillGetTreeNodeFieldValuesXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupName,
    const ConfigurationGroupKey& groupKey, const std::string& startPath, const std::string& modifiedTables,
    const std::string& recordList, const std::string& fieldList) {
  //	setup active tables based on input group and modified tables
  ...
  try {
    ...
    // extract field list
    {
      ...
      __SUP_COUT__ << fieldList << std::endl;
      for (auto& field : fieldPaths) __SUP_COUT__ << "fieldPath " << field << std::endl;
    }
      ...
      while (getline(f, recordUID, ','))  // for each record
      {
        ...
        for (const auto& fieldPath : fieldPaths) {
          ...
          xmlOut.addTextElementToParent(
              "FieldValue", cfgMgr->getNode(startPath + "/" + recordUID + "/" + fieldPath).getValueAsString(),
              parentEl);
        }
      }
    }
...
void ConfigurationGUISupervisor::handleFillTreeNodeCommonFieldsXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupName,
    const ConfigurationGroupKey& groupKey, const std::string& startPath, unsigned int depth,
    const std::string& modifiedTables, const std::string& recordList, const std::string& fieldList) {
  //	setup active tables based on input group and modified tables
  setupActiveTablesXML(xmlOut, cfgMgr, groupName, groupKey, modifiedTables);
...

    {
      ...
      if (fieldList != "") {
        // extract field filter list
        {
          ...
          __SUP_COUT__ << fieldList << std::endl;
          for (auto& field : fieldAcceptList) __SUP_COUT__ << "fieldAcceptList " << field << std::endl;
          for (auto& field : fieldRejectList) __SUP_COUT__ << "fieldRejectList " << field << std::endl;
        }
      }
      ...
      if (recordList == "*")  // handle all records case
      {
        ...
        for (auto& record : records) __SUP_COUT__ << "recordList " << record << std::endl;
      } else if (recordList != "") {
        // extract record list
        {
          std::istringstream f(recordList);
      ...
      retFieldList = startNode.getCommonFields(records, fieldAcceptList, fieldRejectList, depth);
    }

    DOMElement* parentTypeEl;
    for (const auto& fieldInfo : retFieldList) {
      xmlOut.addTextElementToParent("FieldTableName", fieldInfo.tableName_, parentEl);
      xmlOut.addTextElementToParent("FieldColumnName", fieldInfo.columnName_, parentEl);
      xmlOut.addTextElementToParent("FieldRelativePath", fieldInfo.relativePath_, parentEl);
      xmlOut.addTextElementToParent("FieldColumnType", fieldInfo.columnInfo_->getType(), parentEl);
      xmlOut.addTextElementToParent("FieldColumnDataType", fieldInfo.columnInfo_->getDataType(), parentEl);
      xmlOut.addTextElementToParent("FieldColumnDefaultValue", fieldInfo.columnInfo_->getDefaultValue(), parentEl);

      parentTypeEl = xmlOut.addTextElementToParent("FieldColumnDataChoices", "", parentEl);
      ...
      xmlOut.addTextElementToParent("FieldColumnDataChoice",  // add default to list to mimic tree handling
                                    fieldInfo.columnInfo_->getDefaultValue(), parentTypeEl);
      for (const auto& dataChoice : dataChoices)
        xmlOut.addTextElementToParent("FieldColumnDataChoice", dataChoice, parentTypeEl);
    }
  } catch (std::runtime_error& e) {
  ...
  try {
    ...
    if (fieldList != "") {
      // extract field filter list
      {
        ...
        __SUP_COUT__ << fieldList << std::endl;
        for (auto& field : fieldsToGet) __SUP_COUT__ << "fieldsToGet " << field << std::endl;
      }
    }
    ...
    for (auto& field : fieldsToGet) {
      __SUP_COUT__ << "fieldsToGet " << field << std::endl;
      ...
      uniqueValues = cfgMgr->getNode(startPath).getUniqueValuesForField(records, field);

      for (auto& uniqueValue : uniqueValues) {
        __SUP_COUT__ << "uniqueValue " << uniqueValue << std::endl;
...
//			- Note: limitation here is there is no OR among fields/value pairs (in future, could separate
//field/value pairs by : for OR) 		e.g.
...
void ConfigurationGUISupervisor::handleFillTreeViewXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
                                                       ...
                                                       bool hideStatusFalse, const std::string& modifiedTables,
  ...
  setupActiveTablesXML(
      xmlOut, cfgMgr, groupName, groupKey, modifiedTables,
      (startPath ==
  ...
  if (accumulatedErrors != "") xmlOut.addTextElementToData("Warning", accumulatedErrors);
  ...
  try {
    ...
    if (startPath == "/") {
      ...
      if (accumulateTreeErrs != "") xmlOut.addTextElementToData("TreeErrors", accumulateTreeErrs);
    } else {
      ...
      StringMacros::getMapFromString(filterList, filterMap, std::set<char>({';'}) /*pair delimiters*/,
    ...
    for (auto& treePair : rootMap) recursiveTreeToXML(treePair.second, depth - 1, xmlOut, parentEl, hideStatusFalse);
  } catch (std::runtime_error& e) {
...
void ConfigurationGUISupervisor::recursiveTreeToXML(const ConfigurationTree& t, unsigned int depth,
                                                    HttpXmlDocument& xmlOut, DOMElement* parentEl,
  ...
  if (t.isValueNode()) {
    ...
    if (t.getValueType() == ViewColumnInfo::TYPE_FIXED_CHOICE_DATA ||
      ...
      for (const auto& choice : choices) xmlOut.addTextElementToParent("fixedChoice", choice, parentEl);
    }
  } else {
    if (t.isLinkNode()) {
      ...
      if (t.isDisconnected()) {
        ...
        xmlOut.addTextElementToParent((t.isGroupLinkNode() ? "Group" : "U") + std::string("ID"),
                                      t.getDisconnectedLinkID(), parentEl);
        xmlOut.addTextElementToParent("LinkConfigurationName", t.getDisconnectedTableName(), parentEl);
        ...
        for (const auto& choice : choices) xmlOut.addTextElementToParent("fixedChoice", choice, choicesParentEl);
        //}
        // catch(...)
        //{
      ...
      xmlOut.addTextElementToParent((t.isGroupLinkNode() ? "Group" : "U") + std::string("ID"), t.getValueAsString(),
                                    parentEl);

      xmlOut.addTextElementToParent("LinkConfigurationName", t.getConfigurationName(), parentEl);
    ...
    if (depth >= 1) {
      ...
      for (auto& c : C) recursiveTreeToXML(c.second, depth - 1, xmlOut, parentEl, hideStatusFalse);
    }
  }
}
...
//	as xml:
//	<linkToChoice = xxx>
void ConfigurationGUISupervisor::handleGetLinkToChoicesXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
  ...
  try {
    ...
    __SUP_COUT__ << "Failed to find stored version, so attempting to load version: " << version << std::endl;
    cfgMgr->getVersionedConfigurationByName(configName, version);
  }

  if (version != config->getViewVersion()) {
    __SUP_SS__ << "Target table version (" << version << ") is not the currently active version ("
               << config->getViewVersion() << ". Try refreshing the tree." << std::endl;
  ...
  if (linkIdType == "UID") {
    ...
    for (unsigned int row = 0; row < config->getView().getNumberOfRows(); ++row)
      xmlOut.addTextElementToData("linkToChoice", config->getView().getDataView()[row][col]);
  } else if (linkIdType == "GroupID") {
    //		find target column
    //		create the set of values (unique values only)
    //			note: insert group unions individually (i.e. groups | separated)

    std::set<std::string> setOfGroupIDs = config->getView().getSetOfGroupIDs(linkIndex);
    ...
    for (const auto& groupID : setOfGroupIDs) {
      if (!foundInitId && linkInitId == groupID) foundInitId = true;  // mark init id found

      xmlOut.addTextElementToData("linkToChoice", groupID);
    }
    // if init id was not found, add to list
    if (!foundInitId) xmlOut.addTextElementToData("linkToChoice", linkInitId);

    // give all UIDs
    unsigned int col = config->getView().getColUID();
    for (unsigned int row = 0; row < config->getView().getNumberOfRows(); ++row) {
      xmlOut.addTextElementToData("groupChoice", config->getView().getDataView()[row][col]);
      if (config->getView().isEntryInGroup(row, linkIndex, linkInitId))
        xmlOut.addTextElementToData("groupMember", config->getView().getDataView()[row][col]);
    }
  } else {
    __SUP_SS__ << "Unrecognized linkIdType '" << linkIdType << ".'" << std::endl;
    __SS_THROW__;
  }
...
void ConfigurationGUISupervisor::handleMergeGroupsXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupANameContext,
    const ConfigurationGroupKey& groupAKeyContext, const std::string& groupBNameContext,
    const ConfigurationGroupKey& groupBKeyContext, const std::string& groupANameConfig,
    const ConfigurationGroupKey& groupAKeyConfig, const std::string& groupBNameConfig,
    const ConfigurationGroupKey& groupBKeyConfig, const std::string& author, const std::string& mergeApproach) try {
  __SUP_COUT__ << "Merging context group pair " << groupANameContext << " (" << groupAKeyContext << ") & "
               << groupBNameContext << " (" << groupBKeyContext << ") and config group pair " << groupANameConfig
               << " (" << groupAKeyConfig << ") & " << groupBNameConfig << " (" << groupBKeyConfig
  ...
  if (groupANameContext.size() == 0 || groupANameContext[0] == ' ' || groupBNameContext.size() == 0 ||
      groupBNameContext[0] == ' ') {
    skippingContextPair = true;
    __SUP_COUTV__(skippingContextPair);
  }
  if (groupANameConfig.size() == 0 || groupANameConfig[0] == ' ' || groupBNameConfig.size() == 0 ||
      groupBNameConfig[0] == ' ') {
  ...
  if (!skippingContextPair) {
    cfgMgr->loadConfigurationGroup(groupANameContext, groupAKeyContext, false /*doActivate*/, &memberMapAContext,
                                   0 /*progressBar*/, 0 /*accumulateErrors*/, 0 /*groupComment*/, 0 /*groupAuthor*/,
                                   0 /*groupCreationTime*/, false /*doNotLoadMember*/, 0 /*groupTypeString*/
    );
    __SUP_COUTV__(StringMacros::mapToString(memberMapAContext));

    cfgMgr->loadConfigurationGroup(groupBNameContext, groupBKeyContext, false /*doActivate*/, &memberMapBContext,
                                   0 /*progressBar*/, 0 /*accumulateErrors*/, 0 /*groupComment*/, 0 /*groupAuthor*/,
                                   0 /*groupCreationTime*/, false /*doNotLoadMember*/, 0 /*groupTypeString*/
    );
  ...
  if (!skippingConfigPair) {
    cfgMgr->loadConfigurationGroup(groupANameConfig, groupAKeyConfig, false /*doActivate*/, &memberMapAConfig,
                                   0 /*progressBar*/, 0 /*accumulateErrors*/, 0 /*groupComment*/, 0 /*groupAuthor*/,
                                   0 /*groupCreationTime*/, false /*doNotLoadMember*/, 0 /*groupTypeString*/
    );
    __SUP_COUTV__(StringMacros::mapToString(memberMapAConfig));

    cfgMgr->loadConfigurationGroup(groupBNameConfig, groupBKeyConfig, false /*doActivate*/, &memberMapBConfig,
                                   0 /*progressBar*/, 0 /*accumulateErrors*/, 0 /*groupComment*/, 0 /*groupAuthor*/,
                                   0 /*groupCreationTime*/, false /*doNotLoadMember*/, 0 /*groupTypeString*/
    );
      ...
      for (const auto bkey : memberMapBref) {
        ...
        if (memberMapAref.find(bkey.first) == memberMapAref.end()) {
          ...
          ConfigurationVersion newVersion = config->mergeViews(
              cfgMgr->getVersionedConfigurationByName(bkey.first, memberMapAref[bkey.first])->getView(),
              cfgMgr->getVersionedConfigurationByName(bkey.first, bkey.second)->getView(),
              ConfigurationVersion() /* destinationVersion*/, author, mergeApproach /*Rename,Replace,Skip*/,
              uidConversionMap, groupidConversionMap, i == 0 /* fillRecordConversionMaps */,
          ...
          if (i == 1) {
            ...
            try {
              ...
              newVersion =
                  saveModifiedVersionXML(xmlOut, cfgMgr, bkey.first, ConfigurationVersion() /*original source version*/,
                                         false /* makeTemporary */, config, newVersion /*temporary modified version*/,
                                         false /*ignore duplicates*/, true /*look for equivalent*/);
            } catch (std::runtime_error& e) {
              __SUP_SS__
                  << "There was an error saving the '" << config->getConfigurationName()
  ...
  __SUP_SS__ << "Error merging context group pair " << groupANameContext << " (" << groupAKeyContext << ") & "
             << groupBNameContext << " (" << groupBKeyContext << ") and config group pair " << groupANameConfig << " ("
             << groupAKeyConfig << ") & " << groupBNameConfig << " (" << groupBKeyConfig << ") with approach '"
             << mergeApproach << "': \n\n"
             << e.what() << std::endl;
  __SUP_COUT_ERR__ << "\n" << ss.str() << std::endl;
  xmlOut.addTextElementToData("Error", ss.str());
} catch (...) {
  __SUP_SS__ << "Unknown error merging context group pair " << groupANameContext << " (" << groupAKeyContext << ") & "
             << groupBNameContext << " (" << groupBKeyContext << ") and config group pair " << groupANameConfig << " ("
             << groupAKeyConfig << ") & " << groupBNameConfig << " (" << groupBKeyConfig << ") with approach '"
             << mergeApproach << ".' \n\n";
  __SUP_COUT_ERR__ << "\n" << ss.str() << std::endl;
...
void ConfigurationGUISupervisor::handleSavePlanCommandSequenceXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupName,
    const ConfigurationGroupKey& groupKey, const std::string& modifiedTables, const std::string& author,
    const std::string& planName, const std::string& commandString) try {
  ...
  for (const auto& commandPair : IterateConfiguration::commandToTableMap_)
    if (commandPair.second != "")  // skip tables with no parameters
      commandTypeToCommandTableMap.emplace(
          std::pair<std::string, TableEditStruct>(commandPair.first, TableEditStruct(commandPair.second, cfgMgr)));
  ...
  try {
    ...
    std::pair<unsigned int /*link col*/, unsigned int /*link id col*/> commandUidLink;
    {
      ...
      planTable.cfgView_->getChildLink(planTable.cfgView_->findCol(IterateConfiguration::planTableCols_.CommandLink_),
    ...
    // Reset existing plan commands
    {
      ...
      for (unsigned int row = 0; row < planTable.cfgView_->getNumberOfRows(); ++row) {
        ...
        if (planTable.cfgView_->isEntryInGroup(row, groupLinkIndex, groupName)) {
              ...
              commandTypeToCommandTableMap.end())  // skip if invalid command type
          {
            ...
            try {
              ...
              targetGroupName = commandTypeToCommandTableMap[cmdType].cfgView_->getDataView()[cmdRow][cmdCol];

              for (unsigned int trow = 0; trow < targetTable.cfgView_->getNumberOfRows(); ++trow) {
                // remove command from target group..
                if (targetTable.cfgView_->isEntryInGroup(
                        trow, commandTypeToCommandTableMap[cmdType].cfgView_->getColumnInfo(cmdCol).getChildLinkIndex(),
                        targetGroupName)) {
                  __SUP_COUT__ << "Removing target." << std::endl;
                  // remove command entry in plan table
                  if (targetTable.cfgView_->removeRowFromGroup(trow, targetGroupIdCol, targetGroupName,
          ...
          // remove command entry in plan table
          if (planTable.cfgView_->removeRowFromGroup(row, groupIdCol, groupName, true /*deleteRowIfNoGroup*/))
        ...
        while (getline(g, paramSubString, ',')) {
          ...
          } else  // params
          {
            ...
            commands.back().params_.emplace(std::pair<std::string /*param name*/, std::string /*param value*/>(
                paramSubString, StringMacros::decodeURIComponent(paramValue)));
          }
    ...
    for (auto& command : commands) {
      __SUP_COUT__ << "command " << command.type_ << std::endl;
      __SUP_COUT__ << "table " << IterateConfiguration::commandToTableMap_.at(command.type_) << std::endl;
      ...
      // set command type
      planTable.cfgView_->setURIEncodedValue(command.type_, row, cmdTypeCol);

      // set command status true
      planTable.cfgView_->setValueAsString("1", row, planTable.cfgView_->getColStatus());
          ...
          commandTypeToCommandTableMap.end())  // if table exists in map! (some commands may have no parameters)
      {
        ...
        cmdRow = commandTypeToCommandTableMap[command.type_].cfgView_->addRow(author, true /*incrementUniqueData*/,
                                                                              command.type_ + "_COMMAND_");
        ...
        for (auto& param : command.params_) {
          __SUP_COUT__ << "\t param " << param.first << " : " << param.second << std::endl;
          ...
          if (param.first == IterateConfiguration::targetParams_.Tables_) {
            ...
            while (getline(f, targetStr, '=')) {
              ...
              if (!command.targets_.size() || command.targets_.back().table_ != "") {
          ...
          if (param.first == IterateConfiguration::targetParams_.UIDs_) {
            ...
            while (getline(f, targetStr, '=')) {
              __SUP_COUT__ << "\t\t targetStr = " << targetStr << __E__;
              if (!command.targets_.size() || command.targets_.back().UID_ != "") {
                __SUP_COUT__ << "\t\t make targetStr = " << targetStr << __E__;
                // make new target
                command.addTarget();
          ...
          cmdCol = commandTypeToCommandTableMap[command.type_].cfgView_->findCol(param.first);

          __SUP_COUT__ << "param col " << cmdCol << std::endl;

          commandTypeToCommandTableMap[command.type_].cfgView_->setURIEncodedValue(param.second, cmdRow, cmdCol);
        }  // end parameter loop

        cmdUID =
            commandTypeToCommandTableMap[command.type_]
                .cfgView_->getDataView()[cmdRow][commandTypeToCommandTableMap[command.type_].cfgView_->getColUID()];
        ...
        if (command.targets_.size()) {
          ...
          commandTypeToCommandTableMap[command.type_].cfgView_->setValueAsString(IterateConfiguration::TARGET_TABLE,
                                                                                 cmdRow, cmdCol);

          cmdCol = commandTypeToCommandTableMap[command.type_].cfgView_->findCol(
              IterateConfiguration::commandTargetCols_.TargetsLinkGroupID_);
          commandTypeToCommandTableMap[command.type_].cfgView_->setValueAsString(cmdUID + "_Targets", cmdRow, cmdCol);
          ...
          for (const auto& target : command.targets_) {
            ...
            targetTable.cfgView_->addRowToGroup(tgtRow, targetGroupIdCol, cmdUID + "_Targets");

            // set target table
            targetTable.cfgView_->setValueAsString(target.table_, tgtRow, targetTableCol);

            // set target UID
            targetTable.cfgView_->setValueAsString(target.UID_, tgtRow, targetUIDCol);
          }
        }  // end target handling

        // add link at plan level to created UID
        planTable.cfgView_->setValueAsString(commandTypeToCommandTableMap[command.type_].configName_, row,
                                             commandUidLink.first);
        planTable.cfgView_->setValueAsString(cmdUID, row, commandUidLink.second);

        __SUP_COUT__ << "linked to uid = " << cmdUID << std::endl;
    ...
    {
      __SUP_COUT__ << "Erasing temporary version " << planTable.configName_ << "-v" << planTable.temporaryVersion_
                   << std::endl;
    ...
    {
      __SUP_COUT__ << "Erasing temporary version " << targetTable.configName_ << "-v" << targetTable.temporaryVersion_
                   << std::endl;
      // erase with proper version management
    ...
    for (auto& modifiedConfig : commandTypeToCommandTableMap) {
      ...
      {
        __SUP_COUT__ << "Erasing temporary version " << modifiedConfig.second.configName_ << "-v"
                     << modifiedConfig.second.temporaryVersion_ << std::endl;
        // erase with proper version management
        cfgMgr->eraseTemporaryVersion(modifiedConfig.second.configName_, modifiedConfig.second.temporaryVersion_);
      }
    }
  ...
  ConfigurationVersion finalVersion = saveModifiedVersionXML(
      xmlOut, cfgMgr, planTable.configName_, planTable.originalVersion_, true /*make temporary*/, planTable.config_,
      planTable.temporaryVersion_, true /*ignoreDuplicates*/);  // save temporary version properly

  __SUP_COUT__ << "Final plan version is " << planTable.configName_ << "-v" << finalVersion << std::endl;

  finalVersion = saveModifiedVersionXML(xmlOut, cfgMgr, targetTable.configName_, targetTable.originalVersion_,
                                        true /*make temporary*/, targetTable.config_, targetTable.temporaryVersion_,
                                        true /*ignoreDuplicates*/);  // save temporary version properly

  __SUP_COUT__ << "Final target version is " << targetTable.configName_ << "-v" << finalVersion << std::endl;

  for (auto& modifiedConfig : commandTypeToCommandTableMap) {
    if (!modifiedConfig.second.modified_) {
      if (modifiedConfig.second.createdTemporaryVersion_)  // if temporary version created here
      {
        __SUP_COUT__ << "Erasing unmodified temporary version " << modifiedConfig.second.configName_ << "-v"
                     << modifiedConfig.second.temporaryVersion_ << std::endl;
        // erase with proper version management
        cfgMgr->eraseTemporaryVersion(modifiedConfig.second.configName_, modifiedConfig.second.temporaryVersion_);
      }
    ...
    finalVersion = saveModifiedVersionXML(xmlOut, cfgMgr, modifiedConfig.second.configName_,
                                          modifiedConfig.second.originalVersion_, true /*make temporary*/,
                                          modifiedConfig.second.config_, modifiedConfig.second.temporaryVersion_,
                                          true /*ignoreDuplicates*/);  // save temporary version properly

    __SUP_COUT__ << "Final version is " << modifiedConfig.second.configName_ << "-v" << finalVersion << std::endl;
  }
...
void ConfigurationGUISupervisor::handleSaveTreeNodeEditXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
                                                           const std::string& configName, ConfigurationVersion version,
                                                           const std::string& type, const std::string& uid,
                                                           const std::string& colName, const std::string& newValue,
                                                           const std::string& author) try {
  __SUP_COUT__ << "table " << configName << "(" << version << ")" << std::endl;
  ...
  else if (type == "link-UID" || type == "link-GroupID" || type == "value" || type == "value-groupid" ||
           type == "value-bool" || type == "value-bitmap")
    col = config->getView().findCol(colName);
  else if (type == "table" || type == "link-comment" || type == "table-newGroupRow" || type == "table-newUIDRow" ||
           type == "table-newRow")
    ;  // column N/A
  else {
  ...
  if (type == "table" || type == "link-comment") {
    ...
    if (config->getView().isURIEncodedCommentTheSame(newValue)) {
      __SUP_SS__ << "Comment '" << newValue << "' is the same as the current comment. No need to save change."
                 << std::endl;
      __SS_THROW__;
    }
  }
  ...
  try {
    ...
    if (type == "table" || type == "link-comment") {
      ...
      __SUP_COUT__ << "newValue " << linkIndex << "," << groupId << "," << newRowUID << std::endl;
    ...
    } else if (type == "uid" || type == "value" || type == "value-groupid" || type == "value-bool" ||
               type == "value-bitmap") {
      ...
      if (!cfgView->setURIEncodedValue(newValue, row, col, author)) {
        ...
        __SUP_SS__ << "Value '" << newValue
                   << "' is the same as the current value. No need to save change to tree node." << std::endl;
        __SS_THROW__;
      }
      ...
      if (!cfgView->getChildLink(col, isGroup, linkPair)) {
        // not a link ?!
        __SUP_SS__ << "Col '" << colName << "' is not a link column." << std::endl;
        __SS_THROW__;
      }

      __SUP_COUT__ << "linkPair " << linkPair.first << "," << linkPair.second << std::endl;
      ...
      __SUP_COUT__ << "newValue " << newTable << "," << newLinkId << std::endl;
      ...
      if (!cfgView->setURIEncodedValue(newTable, row, linkPair.first, author)) {
        // no change
        __SUP_COUT__ << "Value '" << newTable << "' is the same as the current value." << std::endl;
      } else
        changed = true;

      if (!cfgView->setURIEncodedValue(newLinkId, row, linkPair.second, author)) {
        // no change
        __SUP_COUT__ << "Value '" << newLinkId << "' is the same as the current value." << std::endl;
      } else
        changed = true;
      ...
      if (type == "link-GroupID") {
        ...
        } else  // if changes, save it
        {
          try {
            ...
            saveModifiedVersionXML(xmlOut, cfgMgr, configName, version, true /*make temporary*/, config,
                                   temporaryVersion, true /*ignoreDuplicates*/);  // save temporary version properly
        ...
        csvIndexStart = csvIndex + 1;
        csvIndex = newValue.find(',', csvIndexStart);
        version = ConfigurationVersion(newValue.substr(
            csvIndexStart, csvIndex - csvIndexStart));  // if no more commas will take the rest of string
        ...
        try {
          ...
          __SUP_COUT__ << "Failed to find stored version, so attempting to load version: " << version << std::endl;
          cfgMgr->getVersionedConfigurationByName(newTable, version);
        }
        ...
        for (unsigned int row = 0; row < cfgView->getNumberOfRows(); ++row) {
          ...
          } else if (targetUID == cfgView->getDefaultRowValues()[cfgView->getColUID()] && isInGroup) {
        ...
        } else  // if changes, save it
        {
          try {
            ...
            saveModifiedVersionXML(xmlOut, cfgMgr, newTable, version, true /*make temporary*/, config, temporaryVersion,
                                   true /*ignoreDuplicates*/);  // save temporary version properly
          } catch (std::runtime_error& e)                       // erase temporary view before re-throwing error
          {
        ...
        //	RAR: block because its hard to detect if changes were recently made (one idea: to check if all other
        //values are defaults, to assume it was just created)
        if (0 && !changed && !secondaryChanged && !defaultIsInGroup) {
          __SUP_SS__ << "Link to table '" << newTable << "', linkID '" << newLinkId
                     << "', and selected group members are the same as the current value. "
                     << "No need to save changes to tree." << std::endl;
          __SS_THROW__;
        }

        return;                  // exit since table inits were already tested
      } else if (0 && !changed)  // block error message because sometimes things get setup twice depending on the path
                                 // of the user (e.g. when editing links in tree-view)
      {  // RAR: block also becuase versions are temporary at this point anyway, might as well abuse temporary versions
        __SUP_SS__ << "Link to table '" << newTable << "' and linkID '" << newLinkId
                   << "' are the same as the current values. No need to save change to tree node." << std::endl;
        __SS_THROW__;
      }
    }
  ...
  saveModifiedVersionXML(xmlOut, cfgMgr, configName, version, true /*make temporary*/, config, temporaryVersion,
                         true /*ignoreDuplicates*/);  // save temporary version properly
} catch (std::runtime_error& e) {
...
//		...
//	</configuration>
void ConfigurationGUISupervisor::handleGetConfigurationGroupXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
                                                                const std::string& groupName,
                                                                ConfigurationGroupKey groupKey) try {
  char tmpIntStr[100];
  ...
  if (groupKey.isInvalid() ||  // if invalid or not found, get latest
    ...
    if (sortedKeys.size()) groupKey = *sortedKeys.rbegin();
    __SUP_COUT__ << "Group key requested was invalid or not found, going with latest " << groupKey << std::endl;
  }
  ...
  try {
    ...
    cfgMgr->loadConfigurationGroup(groupName, groupKey, false /*doActivate*/, &memberMap, 0 /*progressBar*/,
                                   &accumulateErrors /*accumulateErrors*/, &groupComment, &groupAuthor,
                                   &groupCreationTime, false /*doNotLoadMember*/, &groupTypeString,
                                   &groupMemberAliases);
  ...
  std::map<std::string, std::map<std::string, ConfigurationVersion>> versionAliases = cfgMgr->getVersionAliases();
  ...
  for (auto& memberPair : memberMap) {
    ...
    if (it == allCfgInfo.end()) {
      xmlOut.addTextElementToData("Error", "Configuration \"" + memberPair.first + "\" can not be retrieved!");
      continue;
    }
    ...
    if (versionAliases.find(it->first) != versionAliases.end())
      for (auto& aliasVersion : versionAliases[it->first])
        xmlOut.addTextElementToParent("ConfigurationExistingVersion",
                                      ConfigurationManager::ALIAS_VERSION_PREAMBLE + aliasVersion.first, configEl);

    for (auto& version : it->second.versions_)
      // if(version == memberPair.second) continue; //CHANGED by RAR on 11/14/2016 (might as well show all versions in
      // list to avoid user confusion)  else
  ...
  for (auto& memberPair : memberMap) {
    ...
    xmlOut.addTextElementToParent("MemberComment",
                                  allCfgInfo.at(memberPair.first).configurationPtr_->getView().getComment(), parentEl);
    // else
    ...
    */
    ...
    for (auto& version:it->second.versions_)
            //if(version == memberPair.second) continue; //CHANGED by RAR on 11/14/2016 (might as well show all versions
    in list to avoid user confusion)
            //else
            xmlOut.addTextElementToParent("ConfigurationExistingVersion", version.toString(), configEl);
    */
  }
...
void ConfigurationGUISupervisor::handleGetConfigurationXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
                                                           const std::string& tableName, ConfigurationVersion version,
  ...
  const std::map<std::string, ConfigurationInfo>& allCfgInfo =  // if allowIllegalColumns, then also refresh
      cfgMgr->getAllConfigurationInfo(allowIllegalColumns, allowIllegalColumns ? &accumulatedErrors : 0,
  ...
  for (auto& configPair : allCfgInfo) {
    xmlOut.addTextElementToData("ExistingConfigurationNames", configPair.first);
      ...
      if (tableIterator != versionAliases.end()) {
        ...
        for (const auto& aliasPair : tableIterator->second) {
          ...
          if (v == aliasPair.second) {
            __SUP_COUT__ << "Found Alias " << aliasPair.second << " --> " << aliasPair.first << __E__;
  ...
  } else  // use view version
  {
    ...
    {
      __SUP_SS__ << "Failed to get table " << tableName << " version " << version << "... defaulting to mock-up! "
                 << std::endl;
    ...
    } catch (...)  // default to mock-up for fail-safe in GUI editor
    {
      __SUP_SS__ << "Failed to get table " << tableName << " version: " << version << "... defaulting to mock-up! "
                 ...
                 << "'Copy and Move' functionality.)" << std::endl;
  ...
  for (int i = 0; i < (int)colInfo.size(); ++i)  // column headers and types
  {
    ...
    if (colInfo[i].getType() == ViewColumnInfo::TYPE_FIXED_CHOICE_DATA ||
        colInfo[i].getType() == ViewColumnInfo::TYPE_BITMAP_DATA || colInfo[i].isChildLink()) {
  ...
  try {
    if (version.isInvalid()) cfgViewPtr->init();
  } catch (std::runtime_error& e) {
  ...
  {
    __SUP_SS__ << "\n\nThere were warnings found when loading the table " << tableName << ":v" << version
               << ". Please see the details below:\n\n"
               << "The source column size was found to be " << cfgViewPtr->getDataColumnSize()
               << ", and the current number of columns for this table is " << cfgViewPtr->getNumberOfColumns()
               ...
               << " table entries missing in " << cfgViewPtr->getNumberOfRows() << " row(s) of data." << std::endl;
...
ConfigurationVersion ConfigurationGUISupervisor::saveModifiedVersionXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& configName,
    ConfigurationVersion originalVersion, bool makeTemporary, ConfigurationBase* config,
    ConfigurationVersion temporaryModifiedVersion, bool ignoreDuplicates, bool lookForEquivalent) {
  ...
  if (!ignoreDuplicates) {
...

    {
      ...
      __SUP_COUT__ << "Filling up cached from " << config->getNumberOfStoredViews() << " to max count of "
                   << config->MAX_VIEWS_IN_CACHE << std::endl;
      for (; config->getNumberOfStoredViews() < config->MAX_VIEWS_IN_CACHE &&
             versionReverseIterator != allCfgInfo.at(configName).versions_.rend();
    ...
    duplicateVersion =
        config->checkForDuplicate(temporaryModifiedVersion,
    ...
    if (lookForEquivalent && !duplicateVersion.isInvalid()) {
      ...
      if (duplicateVersion.isTemporaryVersion() && !makeTemporary) {
        ...
        if (needToEraseTemporarySource) cfgMgr->eraseTemporaryVersion(configName, originalVersion);
    ...
    if (!duplicateVersion.isInvalid()) {
      __SUP_SS__ << "This version is identical to another version currently cached v" << duplicateVersion
...
void ConfigurationGUISupervisor::handleCreateConfigurationXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
                                                              const std::string& configName,
                                                              ConfigurationVersion version, bool makeTemporary,
                                                              const std::string& data, const int& dataOffset,
                                                              const std::string& author, const std::string& comment,
                                                              bool sourceTableAsIs, bool lookForEquivalent) try {
  ...
  if (!version.isInvalid())  // if not using mock-up, then the starting version is the active one
  {
    ...
    if (config->getViewP()->getDataColumnSize() != config->getMockupViewP()->getNumberOfColumns() ||
        config->getViewP()->getSourceColumnMismatch() != 0) {
      __SUP_COUT__ << "config->getViewP()->getNumberOfColumns() " << config->getViewP()->getNumberOfColumns()
                   << std::endl;
      __SUP_COUT__ << "config->getMockupViewP()->getNumberOfColumns() "
                   << config->getMockupViewP()->getNumberOfColumns() << std::endl;
      __SUP_COUT__ << "config->getViewP()->getSourceColumnMismatch() " << config->getViewP()->getSourceColumnMismatch()
                   << std::endl;
  ...
  if (retVal < 0 && (!version.isTemporaryVersion() || makeTemporary) &&
        ...
        !version.isScratchVersion())  // if source version was scratch, then consider it attempt to make it persistent
    {
      ...
      __SUP_COUT__ << "This was interpreted as an attempt to make a persistent version of the scratch table."
                   << std::endl;
    else {
      __SUP_SS__;
    ...
    __SUP_COUT__ << "Allowing the static data because this is converting from temporary to persistent version."
                 << std::endl;
  ...
  saveModifiedVersionXML(xmlOut, cfgMgr, configName, version, makeTemporary, config, temporaryVersion,
                         false /*ignoreDuplicates*/, lookForEquivalent || sourceTableAsIs /*lookForEquivalent*/);
} catch (std::runtime_error& e) {
  __SUP_COUT__ << "Error detected!\n\n " << e.what() << std::endl;
...
//		This configurationManager will have at least empty instances of all base configurations (no null
//pointers) 		and will load the backbone configurations to specified backboneVersion
//
...
//, ConfigurationVersion& backboneVersion)
{
  ...
  __SUP_COUT__ << "Config Session: " << mapKey << " ... out of size: " << userConfigurationManagers_.size()
               << std::endl;
...
//
//
void ConfigurationGUISupervisor::handleCreateConfigurationGroupXML(HttpXmlDocument& xmlOut,
                                                                   ConfigurationManagerRW* cfgMgr,
                                                                   const std::string& groupName,
                                                                   const std::string& configList, bool allowDuplicates,
                                                                   bool ignoreWarnings, const std::string& groupComment,
  ...
  while (c < configList.length()) {
    ...
    if (versionStr.find(ConfigurationManager::ALIAS_VERSION_PREAMBLE) == 0) {
      ...
      if (versionAliases.find(name) != versionAliases.end() &&
        ...
        __SUP_COUT__ << "version alias '" << alias << "'translated to: " << version << std::endl;

        groupAliases[name] = alias;
      } else {
        __SUP_SS__ << "version alias '" << versionStr.substr(ConfigurationManager::ALIAS_VERSION_PREAMBLE.size())
                   << "' was not found in active version aliases!" << std::endl;
        __SUP_COUT_ERR__ << "\n" << ss.str();
        xmlOut.addTextElementToData("Error", ss.str());
        return;
    ...
    if (version.isTemporaryVersion()) {
      __SUP_SS__ << "Groups can not be created using temporary member tables. "
                 << "Table member '" << name << "' with temporary version '" << version << "' is illegal." << std::endl;
      xmlOut.addTextElementToData("Error", ss.str());
      return;
    }
    ...
    if (version.isMockupVersion()) {
      ...
      __SUP_COUT__ << "Creating version from mock-up for name: " << name << " inputVersionStr: " << versionStr
                   << std::endl;
      ...
      // finish off the version creation
      version = saveModifiedVersionXML(
          xmlOut, cfgMgr, name, ConfigurationVersion() /*original source is mockup*/, false /* makeTemporary */, config,
          temporaryVersion /*temporary modified version*/, false /*ignore duplicates*/, true /*look for equivalent*/);
  ...
  if (!allowDuplicates) {
    ...
    ConfigurationGroupKey foundKey = cfgMgr->findConfigurationGroup(groupName, groupMembers, groupAliases);
    ...
    if (!foundKey.isInvalid()) {
      ...
      if (lookForEquivalent) {
        __SUP_COUT__ << "Found equivalent group key (" << foundKey << ") for " << groupName << "." << std::endl;
  ...
  try {
    ...
    for (auto& groupMemberPair : groupMembers) {
      ConfigurationView* cfgViewPtr = cfgMgr->getConfigurationByName(groupMemberPair.first)->getViewP();
      if (cfgViewPtr->getDataColumnSize() != cfgViewPtr->getNumberOfColumns() ||
          cfgViewPtr->getSourceColumnMismatch() != 0)  // check for column size mismatch
      {
        __SUP_SS__ << "\n\nThere were errors found in loading a member table " << groupMemberPair.first << ":v"
                   << cfgViewPtr->getVersion() << ". Please see the details below:\n\n"
                   << "The source column size was found to be " << cfgViewPtr->getDataColumnSize()
                   << ", and the current number of columns for this table is " << cfgViewPtr->getNumberOfColumns()
                   << ". This resulted in a count of " << cfgViewPtr->getSourceColumnMismatch()
                   << " source column mismatches, and a count of " << cfgViewPtr->getSourceColumnMissing()
        ...
        char index = 'a';
        for (auto& srcColName : srcColNames) ss << "\n\t" << index++ << ". " << srcColName;
        ss << std::endl;

        std::set<std::string> destColNames = cfgViewPtr->getColumnStorageNames();
        ss << "\n\nCurrent table column names are as follows:\n";
        index = 'a';
        for (auto& destColName : destColNames) ss << "\n\t" << index++ << ". " << destColName;
        ss << std::endl;
  ...
  try {
    ...
    newKey = cfgMgr->saveNewConfigurationGroup(groupName, groupMembers, groupComment, &groupAliases);
  } catch (std::runtime_error& e) {
...
void ConfigurationGUISupervisor::handleDeleteConfigurationInfoXML(HttpXmlDocument& xmlOut,
  ...
  if (0 == rename((CONFIG_INFO_PATH + configName + CONFIG_INFO_EXT).c_str(),
    ...
    __SUP_COUT_ERR__ << ("Error renaming file to " + (CONFIG_INFO_PATH + configName + CONFIG_INFO_EXT + ".unused"))
                     << std::endl;

    xmlOut.addTextElementToData("Error", ("Error renaming Table Info File to " +
                                          (CONFIG_INFO_PATH + configName + CONFIG_INFO_EXT + ".unused")));
    return;
  }
...
//
void ConfigurationGUISupervisor::handleSaveConfigurationInfoXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
  ...
  } catch (std::runtime_error& e) {  // error! non-alpha
  ...
  if (!allowOverwrite) {
    ...
    if (fp) {
      fclose(fp);
      xmlOut.addTextElementToData("ConfigurationName", configName);
      xmlOut.addTextElementToData("OverwriteError", "1");
      xmlOut.addTextElementToData("Error",
                                  "File already exists! ('" + (CONFIG_INFO_PATH + configName + CONFIG_INFO_EXT) + "')");
      return;
    }
  }
  ...
  outss << "\t\t\t<VIEW Name=\"" << capsName << "\" Type=\"File,Database,DatabaseTest\" Description=\""
        << tableDescription << "\">\n";
  ...
  while (k != (int)(std::string::npos)) {
    ...
    try {
      ...
      xmlOut.addTextElementToData("Error",
                                  std::string("For column name '") + data.substr(i, j - i) + "' - " + e.what());
      return;
    }
  ...
  for (const auto& cfgInfo : allCfgInfo) {
    try {
      ...
      __SUP_COUT_WARN__ << "\n\n##############################################\n"
                        << "Error identified in column info of configuration '" << cfgInfo.first << "':\n\n"
                        << e.what() << "\n\n"
                        << std::endl;
    }
  }
...
void ConfigurationGUISupervisor::handleSetGroupAliasInBackboneXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& groupAlias,
    const std::string& groupName, ConfigurationGroupKey groupKey, const std::string& author) try {
  cfgMgr->loadConfigurationBackbone();
  ...
  for (auto& memberName : backboneMembers) {
    __SUP_COUT__ << "activeVersions[\"" << memberName << "\"]=" << activeVersions[memberName] << std::endl;

    xmlOut.addTextElementToData("oldBackboneName", memberName);
    xmlOut.addTextElementToData("oldBackboneVersion", activeVersions[memberName].toString());
  }
  ...
  try {
    ...
    if (row == (unsigned int)-1)  // if row not found then add a row
    {
      ...
      configView->setValue("This Group Alias was automatically setup by the server.", row, col);
    ...
    __SUP_COUT__ << "\t\t groupName: " << groupName << " vs " << configView->getDataView()[row][col] << std::endl;
    ...
    __SUP_COUT__ << "\t\t groupKey: " << groupKey << " vs " << configView->getDataView()[row][col] << std::endl;
  ...
  if (isDifferent)  // make new version if different
  {
    ...
    newAssignedVersion =
        saveModifiedVersionXML(xmlOut, cfgMgr, config->getConfigurationName(), originalVersion, false /*makeTemporary*/,
...
void ConfigurationGUISupervisor::handleSetVersionAliasInBackboneXML(
    HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr, const std::string& versionAlias,
    const std::string& configName, ConfigurationVersion version, const std::string& author) try {
  cfgMgr->loadConfigurationBackbone();
  std::map<std::string, ConfigurationVersion> activeVersions = cfgMgr->getActiveVersions();
  ...
  try {
    ...
    try {
      ...
      do {  // start looking from beyond last find
    ...
    if (row == (unsigned int)-1)  // if row not found then add a row
    {
      ...
      configView->setValue(configName.substr(0, configName.rfind("Configuration")) + versionAlias, row, col);
    ...
    __SUP_COUT__ << "\t\t version: " << version << " vs " << configView->getDataView()[row][col] << std::endl;
...
void ConfigurationGUISupervisor::handleAliasGroupMembersInBackboneXML(
  ...
  ConfigurationBase* config = cfgMgr->getConfigurationByName(versionAliasesTableName);
  ConfigurationVersion temporaryVersion = config->createTemporaryView(activeVersions[versionAliasesTableName]);
  ...
  std::map<std::string /*name*/, ConfigurationVersion /*version*/> memberMap;
  try {
    cfgMgr->loadConfigurationGroup(groupName, groupKey, false /*doActivate*/, &memberMap, 0, 0, 0, 0, 0,  // defaults
                                   true /*doNotLoadMember*/);
  } catch (...) {
  ...
  for (auto& memberPair : memberMap) {
    ...
    __SUP_COUT__ << "Adding alias for " << memberPair.first << "_v" << memberPair.second << std::endl;
    ...
    if (row == (unsigned int)-1)  // if row not found then add a row
    {
      ...
      configView->setValue(memberPair.first.substr(0, memberPair.first.rfind("Configuration")) + versionAlias, row,
                           col);
    ...
    __SUP_COUT__ << "\t\t version: " << memberPair.second << " vs " << configView->getDataView()[row][col] << std::endl;
    ...
    if (thisMemberIsDifferent) isDifferent = true;
  }
  ...
  if (isDifferent)  // make new version if different
  {
    ...
    newAssignedVersion = cfgMgr->saveNewConfiguration(versionAliasesTableName, temporaryVersion);
  } else  // use existing version
  {
    __SUP_COUT__ << "\t\t**************************** Using existing table version" << std::endl;
...
//		...
//
void ConfigurationGUISupervisor::handleGroupAliasesXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr) {
  cfgMgr->loadConfigurationBackbone();
  std::map<std::string, ConfigurationVersion> activeVersions = cfgMgr->getActiveVersions();
  ...
  __SUP_COUT__ << "activeVersions[\"" << groupAliasesTableName << "\"]=" << activeVersions[groupAliasesTableName]
               << std::endl;
  xmlOut.addTextElementToData("GroupAliasesConfigurationName", groupAliasesTableName);
  xmlOut.addTextElementToData("GroupAliasesConfigurationVersion", activeVersions[groupAliasesTableName].toString());
  ...
  for (auto& aliasNodePair : aliasNodePairs) {
    ...
    xmlOut.addTextElementToData("AliasComment", aliasNodePair.second.getNode("CommentDescription").getValueAsString());

    // get group comment
    groupComment = "";  // clear just in case failure
    groupType = "Invalid";
    try {
      cfgMgr->loadConfigurationGroup(groupName, ConfigurationGroupKey(groupKey), 0, 0, 0, 0, &groupComment, 0,
                                     0,  // mostly defaults
...
//		...
//
void ConfigurationGUISupervisor::handleVersionAliasesXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr) {
  cfgMgr->loadConfigurationBackbone();
  std::map<std::string, ConfigurationVersion> activeVersions = cfgMgr->getActiveVersions();
  ...
  __SUP_COUT__ << "activeVersions[\"" << versionAliasesTableName << "\"]=" << activeVersions[versionAliasesTableName]
               << std::endl;
  xmlOut.addTextElementToData("VersionAliasesVersion", activeVersions[versionAliasesTableName].toString());
  ...
  for (auto& aliasNodePair : aliasNodePairs) {
    ...
    xmlOut.addTextElementToData("VersionAlias", aliasNodePair.second.getNode("VersionAlias").getValueAsString());
    xmlOut.addTextElementToData("ConfigurationName",
                                aliasNodePair.second.getNode("ConfigurationName").getValueAsString());
    xmlOut.addTextElementToData("Version", aliasNodePair.second.getNode("Version").getValueAsString());
    xmlOut.addTextElementToData("Comment", aliasNodePair.second.getNode("CommentDescription").getValueAsString());
  }
}
...
//		...
//
void ConfigurationGUISupervisor::handleConfigurationGroupsXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
                                                              bool returnMembers) {
  DOMElement* parentEl;
    ...
    for (auto& keyInSet : groupInfo.second.keys_) {
      ...
      if (returnMembers) {
        ...
        if (loadingHistoricalInfo) {
          groupComment = "";  // clear just in case failure
          try {
            cfgMgr->loadConfigurationGroup(groupName, keyInSet, 0, 0, 0, 0, &groupComment, 0, 0,  // mostly defaults
                                           true /*doNotLoadMembers*/, &groupTypeString);
          } catch (...) {
            groupTypeString = "Invalid";
...
//		...
//
void ConfigurationGUISupervisor::handleConfigurationsXML(HttpXmlDocument& xmlOut, ConfigurationManagerRW* cfgMgr,
  ...
  while (it != allCfgInfo.end()) {
    ...
    if (versionAliases.find(it->first) != versionAliases.end())
      for (auto& aliasVersion : versionAliases[it->first])
        if (it->second.versions_.find(aliasVersion.second) != it->second.versions_.end())
          // if(aliasVersion.first != ConfigurationManager::SCRATCH_VERSION_ALIAS) //NOT NEEDED IF SCRATCH IS ALWAYS
          // ALIAS
          xmlOut.addTextElementToParent("Version", ConfigurationManager::ALIAS_VERSION_PREAMBLE + aliasVersion.first,
                                        parentEl);
    //				else //NOT NEEDED IF SCRATCH IS ALWAYS ALIAS
    //					__SUP_COUT_ERR__ << "Alias for table " << it->first << " is a reserved alias '"
    //<<
    //						ConfigurationManager::SCRATCH_VERSION_ALIAS << "' - this is illegal." <<
    //std::endl;
    ...
    for (auto& version : it->second.versions_)
      if (!version.isScratchVersion()) xmlOut.addTextElementToParent("Version", version.toString(), parentEl);
