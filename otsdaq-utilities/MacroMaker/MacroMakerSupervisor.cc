...
//========================================================================================================================
MacroMakerSupervisor::MacroMakerSupervisor(xdaq::ApplicationStub* stub) : CoreSupervisorBase(stub) {
  INIT_MF("MacroMaker");
  ...
  xoap::bind(this, &MacroMakerSupervisor::frontEndCommunicationRequest, "FECommunication", XDAQ_NS_URI);
...
//========================================================================================================================
void MacroMakerSupervisor::request(const std::string& requestType, cgicc::Cgicc& cgiIn, HttpXmlDocument& xmlOut,
                                   const WebUsers::RequestUserInfo& userInfo) try {
  __SUP_COUT__ << "User name is " << userInfo.username_ << "." << __E__;
  __SUP_COUT__ << "User permission level for request '" << requestType << "' is " << unsigned(userInfo.permissionLevel_)
               << "." << __E__;
  ...
  __SS__ << "Error occurred handling request '" << requestType << "': " << e.what() << __E__;
  __SUP_COUT__ << ss.str();
...
//========================================================================================================================
xoap::MessageReference MacroMakerSupervisor::frontEndCommunicationRequest(xoap::MessageReference message) try {
  ...
  if (type == "initFElist")  // gateway initializes during configure
  {
    ...
    cfgMgr.loadConfigurationGroup(groupName, ConfigurationGroupKey(groupKey), true);
    ...
    for (auto& feApp : feTypeSupervisors) {
      __SUP_COUT__ << "FEs for app " << feApp.first << ":" << feApp.second.getName() << __E__;
             ...
             type == "macroMultiDimensionalCheck")      // from iterator
  {
    ...
    if (it == allFESupervisorInfo_.end()) {
      __SUP_SS__ << "Error transmitting request to FE Supervisor '" << targetInterfaceID << ":" << FESupervisorIndex
                 << ".' \n\n"
    ...
    try {
      __SUP_COUT__ << "Forwarding request: " << SOAPUtilities::translate(message) << __E__;

      xoap::MessageReference replyMessage = SOAPMessenger::sendWithSOAPReply(it->second.getDescriptor(), message);

      if (type != "feSend") {
        __SUP_COUT__ << "Forwarding FE Macro response: " << SOAPUtilities::translate(replyMessage) << __E__;

        return replyMessage;
      }
    } catch (const xdaq::exception::Exception& e) {
      __SUP_SS__ << "Error forwarding FE Communication request to FE Supervisor '" << targetInterfaceID << ":"
                 << FESupervisorIndex << ".' "
  ...
  xoap::MessageReference returnMessage = SOAPUtilities::makeSOAPMessageReference("Error");
...
void MacroMakerSupervisor::getFElist(HttpXmlDocument& xmldoc) {
  ...
  for (auto& appInfo : allFESupervisorInfo_) {
    ...
    __SUP_COUT__ << "FESupervisor LID = " << appInfo.second.getId() << " name = " << appInfo.second.getName() << __E__;

    try {
      xoap::MessageReference retMsg =
          SOAPMessenger::sendWithSOAPReply(appInfo.second.getDescriptor(), "MacroMakerSupervisorRequest", txParameters);
...
void MacroMakerSupervisor::writeData(HttpXmlDocument& xmldoc, cgicc::Cgicc& cgi, const std::string& username) {
  ...
  for (unsigned int i = 0; i < supervisorIndices.size(); i++) {
    ...
    __SUP_COUT__ << "The index of the supervisor instance is: " << FESupervisorIndex << __E__;
    __SUP_COUT__ << "...and the interface ID is: " << interfaceIndex << __E__;

    SupervisorInfoMap::iterator it = allFESupervisorInfo_.find(FESupervisorIndex);
    if (it == allFESupervisorInfo_.end()) {
      __SUP_SS__ << "Error transmitting request to FE Supervisor '" << interfaceIndex << ":" << FESupervisorIndex
                 << ".' \n\n"
    ...
    try {
      xoap::MessageReference replyMessage =
          SOAPMessenger::sendWithSOAPReply(it->second.getDescriptor(), "MacroMakerSupervisorRequest", txParameters);

      __SUP_COUT__ << "Response received: " << SOAPUtilities::translate(replyMessage) << __E__;
...
void MacroMakerSupervisor::readData(HttpXmlDocument& xmldoc, cgicc::Cgicc& cgi, const std::string& username) {
  ...
  rxParameters.addParameter("Error");
  __SUP_COUT__ << "Here comes the array from multiselect box for READ, behold: " << supervisorIndexArray << ","
               << interfaceIndexArray << __E__;
  ...
  for (unsigned int i = 0; i < supervisorIndices.size(); i++) {
    ...
    try {
      ...
      __SUP_COUT__ << "Response received: " << SOAPUtilities::translate(retMsg) << __E__;
...
void MacroMakerSupervisor::createMacro(HttpXmlDocument& xmldoc, cgicc::Cgicc& cgi, const std::string& username) {
  ...
  std::string Notes = CgiDataUtilities::decodeURIComponent(CgiDataUtilities::postData(cgi, "Notes"));
...
void MacroMakerSupervisor::loadMacro(const std::string& macroName, std::string& macroString,
                                     const std::string& username /*=""*/) {
  __SUP_COUTV__(macroName);
  ...
  if (macroString == "") {
    __SUP_SS__ << "Unable to locate file for macro '" << macroName << "'... does it exist?" << __E__;
    if (username != "") ss << " Attempted username was '" << username << ".'" << __E__;
    __SUP_SS_THROW__;
  }
...
void MacroMakerSupervisor::loadMacros(HttpXmlDocument& xmldoc, const std::string& username) {
  ...
  if ((dir = opendir(fullPath.c_str())) != NULL) {
    ...
    __SUP_COUT__ << "Looping through privateMacros folder failed! Wrong directory" << __E__;
  }
  fullPath = (std::string)MACROS_DB_PATH + "publicMacros/";
  returnStr = "";
  if ((dir = opendir(fullPath.c_str())) != NULL) {
...
//========================================================================================================================
void MacroMakerSupervisor::appendCommandToHistory(std::string Command, std::string Format, std::string Time,
                                                  std::string Interfaces, const std::string& username) {
...
void MacroMakerSupervisor::loadHistory(HttpXmlDocument& xmldoc, const std::string& username) {
  ...
  if (read.is_open()) {
    ...
    if (fileSz > MAX_HISTORY_SIZE) {
      ...
      if (!fp) {
        __SS__ << "Big problem with macromaker history file: " << fileName << __E__;
        __SS_THROW__;
      }
    ...
    if (fileSz > 1) returnStr[fileSz - 2] = '\0';  // remove final newline and last #
...
void MacroMakerSupervisor::editMacro(HttpXmlDocument& xmldoc, cgicc::Cgicc& cgi, const std::string& username) {
  ...
  {
    int result;
    result = rename((MACROS_DB_PATH + username + "/" + oldMacroName + ".dat").c_str(),
...
//========================================================================================================================
void MacroMakerSupervisor::exportFEMacro(HttpXmlDocument& xmldoc, cgicc::Cgicc& cgi, const std::string& username) {
  std::string macroName = CgiDataUtilities::getData(cgi, "MacroName");
  ...
  std::string macroNotes = CgiDataUtilities::decodeURIComponent(CgiDataUtilities::postData(cgi, "MacroNotes"));
  ...
  std::map<std::string /*special type*/, std::set<std::string> /*special file paths*/> specialsCodeMap =
  ...
  if (!foundHeaderFile) {
    __SS__ << "Could not find the header file for the FE Interface plugins at '" << headerFile << ".' Does MacroMaker "
           << "have access to the source code? Check that the Supervisor context places MacroMaker in a "
           << "location with access to the source code." << __E__;
    __SS_THROW__;
  }
  if (!foundSourceFile) {
    __SS__ << "Could not find the source file for the FE Interface plugins at '" << sourceFile << ".' Does MacroMaker "
           << "have access to the source code? Check that the Supervisor context places MacroMaker in a "
           << "location with access to the source code." << __E__;
    __SS_THROW__;
  }
  ...
  createCode(codess, commands, "\t" /*tabOffset*/, true /*forFeMacro*/, &inArgNames, &outArgNames);
  ...
  // find start of constructor and register macro
  {
    ...
    if (insertPos == std::string::npos) {
      __SS__ << "Could not find the code insert position in the source file '" << sourceFile
             << ".' The FE plugin class constructor must be '" << pluginName << ":" << pluginName
    ...
    insert = "\n\t//registration of FEMacro '" + macroName + "' generated, " + timeBuffer + ", by '" + username +
  ...
  // find end of source to append FE Macro function
  {
    ...
    if (insertPos == std::string::npos) {
      __SS__ << "Could not find the code insert position in the source file '" << sourceFile
             << ".' The FE plugin class must end with a 'DEFINE_OTS_INTERFACE(" << pluginName
             << ")' - is this the case?" << __E__;
      __SS_THROW__;
    }
    __SUP_COUTV__(insertPos);
    ...
    insert =
        ...
        "=============================================================================================================="
        "==========\n//" +
        macroName + "\n" + "//\tFEMacro '" + macroName + "' generated, " + timeBuffer + ", by '" + username +
        "' using MacroMaker.\n" + "//\tMacro Notes: " + macroNotes + "\n" + "void " + pluginName + "::" + macroName +
  ...
  // find end of class by looking for last };
  {
    ...
    if (insertPos == std::string::npos) {
      __SS__ << "Could not find the code insert position in the header file '" << headerFile
    ...
    insert = "\npublic: // FEMacro '" + macroName + "' generated, " + timeBuffer + ", by '" + username +
             "' using MacroMaker.\n\t" + "void " + macroName + "\t(__ARGS__);\n";
...
//========================================================================================================================
// createCode
void MacroMakerSupervisor::createCode(std::ostream& out, const std::vector<std::string>& commands,
                                      const std::string& tabOffset, bool forFeMacro, std::set<std::string>* inArgNames,
  ...
  out << "\n"
      << tabOffset << "\t"
      << "char *address \t= new char[universalAddressSize_]{0};	//create address buffer of interface size and "
         "init to all 0";
  out << "\n"
      << tabOffset << "\t"
      << "char *data \t\t= new char[universalDataSize_]{0};		//create data buffer of interface size and init "
         "to all 0";
  ...
  out << "\n"
      << tabOffset << "\t"
      << "std::map<std::string /*arg name*/,uint64_t /*arg val*/> macroArgs; //create map from arg name to 64-bit "
         "number";
  ...
  for (unsigned int i = 0; i < commands.size(); i++) {
    ...
    if (oneCommand[1][0] == 'w' || oneCommand[1][0] == 'r') {
      ...
      out << tabOffset << "\t"
          << "__CFG_COUT__ << \"Sleeping for... \" << " << oneCommand[2] << " << \" milliseconds \" << __E__;\n";
      out << tabOffset << "\t"
          << "usleep(" << oneCommand[2] << "*1000 /* microseconds */);\n";
      continue;
    } else {
      __SS__ << "FATAL ERROR: Unknown command '" << oneCommand[1] << "'... command is not w, r or d" << __E__;
      __SS_THROW__;
    }
          ...
          argInHasBeenInitializedSet.end())  // only initialize input argument once
      {
        ...
        if (!forFeMacro) {
          ...
          out << tabOffset << "\t"
              ...
              << "\n"
              << tabOffset << "\t\t\"" << oneCommand[2] << "\").getValue<uint64_t>();";
        } else {
          if (inArgNames) inArgNames->emplace(oneCommand[2]);

          // get address from arguments
          out << tabOffset << "\t"
              << "macroArgs[\"" << oneCommand[2] << "\"] = __GET_ARG_IN__(\"" << oneCommand[2] << "\", uint64_t);";
        }
      }
    ...
    if (oneCommand[1] == "w")  // if write, handle data too
    {
      if (dataIsVariable)  // handle data as variable
      {
            ...
            argInHasBeenInitializedSet.end())  // only initialize input argument once
        {
          ...
          if (forFeMacro) {
            ...
            out << "\n"
                << tabOffset << "\t"
                << "macroArgs[\"" << oneCommand[3] << "\"] = __GET_ARG_IN__(\"" << oneCommand[3]
      ...
      out << tabOffset << "\t"
          << "memcpy(&macroArgs[\"" << outputArgName << "\"],data,8); //copy buffer to argument map";

      // copy read data to output args
      if (forFeMacro)
        out << "\n"
            << tabOffset << "\t"
            << "__SET_ARG_OUT__(\"" << outputArgName << "\",macroArgs[\"" << outputArgName
...
//	return -1 size
std::string MacroMakerSupervisor::generateHexArray(const std::string& sourceHexString, int& numOfBytes) {
  ...
  for (int i = 0; i < numOfBytes * 2; i += 2) {
    // detect non-hex
    if (!((srcHexStr[i] >= '0' && srcHexStr[i] <= '9') || (srcHexStr[i] >= 'a' && srcHexStr[i] <= 'f') ||
    ...
    if (i != 0) retSs << ", ";
    retSs << "0x" << srcHexStr[srcHexStr.size() - 1 - i - 1] << srcHexStr[srcHexStr.size() - 1 - i];
  }
  retSs << "};";
...
void MacroMakerSupervisor::runFEMacro(HttpXmlDocument& xmldoc, cgicc::Cgicc& cgi) {
  ...
  if (supervisorDescriptorPairIt == allFESupervisorInfo_.end()) {
    __SS__ << "Targeted Supervisor Descriptor was not found. Attempted target "
           << "was UID=" << feUID << " at feSupervisorID=" << feSupervisorID << "." << __E__;
    __SUP_COUT_ERR__ << "\n" << ss.str();
  ...
  xoap::MessageReference retMsg =
      SOAPMessenger::sendWithSOAPReply(supervisorDescriptorPairIt->second.getDescriptor(),  // supervisor descriptor
