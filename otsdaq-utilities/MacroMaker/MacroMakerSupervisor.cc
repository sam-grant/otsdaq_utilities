...
//========================================================================================================================
void MacroMakerSupervisor::request (const std::string& requestType, cgicc::Cgicc& cgiIn, HttpXmlDocument& xmlOut, const WebUsers::RequestUserInfo& userInfo) try
{
	__SUP_COUT__ << "User name is " << userInfo.username_ << "." << __E__;
	__SUP_COUT__ << "User permission level for request '" << requestType << "' is " << unsigned(userInfo.permissionLevel_) << "." << __E__;
...
xoap::MessageReference MacroMakerSupervisor::frontEndCommunicationRequest (xoap::MessageReference message) try
{
	...
	if (type == "initFElist")  //gateway initializes during configure
	{
		...
		for (auto& feApp : feTypeSupervisors)
		{
			...
			std::vector<std::string> feChildren =
			    appsNode.getNode (feApp.second.getName ()).getNode ("LinkToSupervisorTable").getNode ("LinkToFEInterfaceTable").getChildrenNames ();
	         ...
	         type == "macroMultiDimensionalCheck")      //from iterator
	{
		...
		{
			__SUP_SS__ << "Destination front end interface ID '" << targetInterfaceID << "' was not found in the list of front ends." << __E__;
			__SUP_SS_THROW__;
		}
		...
		{
			__SUP_SS__ << "Error forwarding FE Communication request to FE Supervisor '" << targetInterfaceID << ":" << FESupervisorIndex << ".' "
...
void MacroMakerSupervisor::getFElist (HttpXmlDocument& xmldoc)
{
	...
	for (auto& appInfo : allFESupervisorInfo_)
	{
		...
		{
			__SS__ << "Error transmitting request to FE Supervisor LID = " << appInfo.second.getId () << " name = " << appInfo.second.getName () << ". \n\n"
			       << e.what () << __E__;
			__SUP_COUT_ERR__ << ss.str ();
			return;
		}
...
void MacroMakerSupervisor::exportFEMacro (HttpXmlDocument& xmldoc, cgicc::Cgicc& cgi, const std::string& username)
{
	...
	if (contents.find (pluginName + "::" + macroName) !=
	    std::string::npos)
	{
		__SS__ << "The function definition '" << (pluginName + "::" + macroName) << "(...)' already exists in the source file '" << sourceFile << ".' Duplicate functions are not allowed - please rename the macro or modify the source file." << __E__;
		__SS_THROW__;
	}
	...
	//find start of constructor and register macro
	{
		...
		{
			__SS__ << "Could not find the code insert position in the source file '" << sourceFile << ".' The FE plugin class constructor must be '" << pluginName << ":" << pluginName << "' - is this the case?" << __E__;
			__SS_THROW__;
		}
		...
		if (insertPos == std::string::npos)
		{
			__SS__ << "Could not find the code insert position in the source file '" << sourceFile << ".' The FE plugin class constructor must begin with '{"
	...
	//find end of source to append FE Macro function
	{
		...
		if (insertPos == std::string::npos)
		{
			__SS__ << "Could not find the code insert position in the source file '" << sourceFile << ".' The FE plugin class must end with a 'DEFINE_OTS_INTERFACE(" << pluginName << ")' - is this the case?" << __E__;
			__SS_THROW__;
		}
		__SUP_COUTV__ (insertPos);
	...
	//find end of class by looking for last };
	{
		...
		if (insertPos == std::string::npos)
		{
			__SS__ << "Could not find the code insert position in the header file '" << headerFile << ".' The FE plugin class must end with a '};' - is this the case?" << __E__;
			__SS_THROW__;
		}
	...
	for (unsigned int i = 0; i < commands.size (); i++)
	{
		...
		if (addressIsVariable)  //handle address as variable
		{
			    ...
			    argInHasBeenInitializedSet.end ())  //only initialize input argument once
			{
				...
				if (!forFeMacro)
				{
					...
					out << tabOffset << "\t"
					    << "macroArgs[\"" << oneCommand[2] << "\"] = theXDAQContextConfigTree_.getNode(theConfigurationPath_).getNode("
					    << "\n"
			...
			out << "\n"
			    << tabOffset << "\tmemcpy(address,&macroArgs[\"" << oneCommand[2] << "\"],8); //copy macro address argument to buffer";
		}
			...
			if (dataIsVariable)  //handle data as variable
			{
				    ...
				    argInHasBeenInitializedSet.end ())  //only initialize input argument once
				{
					...
					if (forFeMacro)
					{
						...
						out << "\n"
						    << tabOffset << "\t"
						    << "macroArgs[\"" << oneCommand[3] << "\"] = __GET_ARG_IN__(\"" << oneCommand[3] << "\", uint64_t); //initialize from input arguments";
					}
					else
					{
						...
						out << "\n"
						    << tabOffset << "\t"
						    << "macroArgs[\"" << oneCommand[3] << "\"] = theXDAQContextConfigTree_.getNode(theConfigurationPath_).getNode("
						    << "\n"
						    << tabOffset << "\t\t\"" << oneCommand[3] << "\").getValue<uint64_t>(); //initialize from configuration tree";
					}
				}
				out << "\t//get macro data argument";
				out << "\n"
				    << tabOffset << "\tmemcpy(data,&macroArgs[\"" << oneCommand[3] << "\"],8); //copy macro data argument to buffer";
			}
		...
		else
		{
			...
			if (forFeMacro)
				out << "\n"
				    << tabOffset << "\t"
				    << "__SET_ARG_OUT__(\"" << outputArgName << "\",macroArgs[\"" << outputArgName << "\"]); //update output argument result";
