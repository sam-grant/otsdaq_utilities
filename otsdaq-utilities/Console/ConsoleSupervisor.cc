...
void ConsoleSupervisor::messageFacilityReceiverWorkLoop (ConsoleSupervisor *cs)
{
	...
	catch (...)
	{
		...
		{
			__SS__ << "FATAL Console error. Could not initialize socket on port " << myport << ". Perhaps the port is already in use? Check for multiple stale instances of otsdaq processes, or notify admins."
		...
		__SS__ << "FATAL Console error. Could not initialize socket on port " << myport << ". Perhaps it is already in use? Exiting Console receive loop." << std::endl;
		__COUT__ << ss.str ();
	...
	while (1)
	{
		...
		if (rsock.receive (buffer, 1 /*timeoutSeconds*/, 0 /*timeoutUSeconds*/, false /*verbose*/) != -1)
		{
			...
			if (newSourceId != -1 && sourceLastSequenceID.find (newSourceId) != sourceLastSequenceID.end () &&  //ensure not first packet received
			    ((newSequenceId == 0 &&
			     ...
			     newSequenceId != sourceLastSequenceID[newSourceId] + 1))    //normal sequence case
			{
				...
				__SS__ << "Missed packets from " << cs->messages_[cs->writePointer_].getSource () << "! Sequence IDs " << sourceLastSequenceID[newSourceId] << " to " << newSequenceId << "." << std::endl;
				std::cout << ss.str ();
		...
		else
		{
			    ...
			    (heartbeatCount < 60 * 5 && heartbeatCount % 60 == 59))  //every ~2 min for first 5 messages
			{
				...
				__MOUT__ << "Console is alive and waiting... (if no messages, next heartbeat is in approximately two minutes)" << std::endl;
			}
			else if (heartbeatCount % (60 * 30) == 59)  //approx every hour
			{
				++selfGeneratedMessageCount;  //increment internal message count
				__MOUT__ << "Console is alive and waiting a long time... (if no messages, next heartbeat is in approximately one hour)" << std::endl;
			}
...
void ConsoleSupervisor::defaultPage (xgi::Input *in, xgi::Output *out)
{
	...
	*out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame src='/WebPath/html/Console.html?urn=" << getApplicationDescriptor ()->getLocalId () << "'></frameset></html>";
}
...
void ConsoleSupervisor::request (const std::string &requestType, cgicc::Cgicc &cgiIn, HttpXmlDocument &xmlOut, const WebUsers::RequestUserInfo &userInfo)
{
	...
	if (requestType == "GetConsoleMsgs")
	{
		...
		{
			__SUP_COUT_ERR__ << "Invalid Parameters! lastUpdateCount=" << lastUpdateCountStr << ", lastUpdateIndex=" << lastUpdateIndexStr << std::endl;
                                              ...
                                              const unsigned int lastUpdateIndex)
{
	...
	{
		__SS__ << "Invalid lastUpdateIndex: " << lastUpdateIndex << " messagesArray size = " << messages_.size () << std::endl;
		__SS_THROW__;
	}
	     ...
	     refreshReadPointer_ = (refreshReadPointer_ + 1) % messages_.size ())
	{
		if (messages_[refreshReadPointer_].getCount () < lastUpdateCount)
		{
			if (!requestOutOfSync)  //record out of sync message once only
			{
				...
				__SS__ << "Request is out of sync! Message count should be more recent than update clock! " << messages_[refreshReadPointer_].getCount () << " < " << lastUpdateCount << std::endl;
