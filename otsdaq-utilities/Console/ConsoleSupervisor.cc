...
void ConsoleSupervisor::messageFacilityReceiverWorkLoop(ConsoleSupervisor *cs) {
  ...
  if (!fp) {
    __SS__ << "File with port info could not be loaded: " << QUIET_CFG_FILE << std::endl;
    __COUT__ << "\n" << ss.str();
    __SS_THROW__;
  }
  char tmp[100];
  ...
  try {
    ...
    if (1)  // generate special message and throw for failed socket
    {
      __SS__ << "FATAL Console error. Could not initialize socket on port " << myport
             << ". Perhaps the port is already in use? Check for multiple stale instances of otsdaq processes, or "
                "notify admins."
             << " Multiple instances of otsdaq on the same node should be possible, but port numbers must be unique."
             << std::endl;
      __SS_THROW__;
    }
    ...
    cs->messages_[cs->writePointer_].set(CONSOLE_SPECIAL_ERROR + ss.str(), cs->messageCount_++);
  ...
  std::map<unsigned int, unsigned int>
      sourceLastSequenceID;  // map from sourceID to lastSequenceID to identify missed messages
  ...
  while (1) {
    ...
    if (rsock.receive(buffer, 1 /*timeoutSeconds*/, 0 /*timeoutUSeconds*/, false /*verbose*/) != -1) {
      if (i != 200) {
      ...
      if (newSourceId != -1 &&
          sourceLastSequenceID.find(newSourceId) != sourceLastSequenceID.end() &&  // ensure not first packet received
          ((newSequenceId == 0 && sourceLastSequenceID[newSourceId] != (unsigned int)-1) ||  // wrap around case
           newSequenceId != sourceLastSequenceID[newSourceId] + 1))                          // normal sequence case
      {
        // missed some messages!
        __SS__ << "Missed packets from " << cs->messages_[cs->writePointer_].getSource() << "! Sequence IDs "
               << sourceLastSequenceID[newSourceId] << " to " << newSequenceId << "." << std::endl;
        std::cout << ss.str();
        ...
        cs->messages_[cs->writePointer_].set(CONSOLE_SPECIAL_WARNING + ss.str(), cs->messageCount_++);
      }
          ...
          (heartbeatCount < 60 * 5 && heartbeatCount % 60 == 59))  // every ~2 min for first 5 messages
      {
        ...
        __MOUT__ << "Console is alive and waiting... (if no messages, next heartbeat is in approximately two minutes)"
                 << std::endl;
      } else if (heartbeatCount % (60 * 30) == 59)  // approx every hour
      {
        ++selfGeneratedMessageCount;  // increment internal message count
        __MOUT__ << "Console is alive and waiting a long time... (if no messages, next heartbeat is in approximately "
                    "one hour)"
                 << std::endl;
      }
    ...
    if (i == 120 || selfGeneratedMessageCount == 5) {
      __COUT__ << "No messages received at Console Supervisor. Exiting Console messageFacilityReceiverWorkLoop"
               << std::endl;
...
void ConsoleSupervisor::forceSupervisorPropertyValues() {
  CorePropertySupervisorBase::setSupervisorProperty(
      CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.AutomatedRequestTypes, "GetConsoleMsgs");
...
void ConsoleSupervisor::request(const std::string &requestType, cgicc::Cgicc &cgiIn, HttpXmlDocument &xmlOut,
  ...
  if (requestType == "GetConsoleMsgs") {
    ...
    std::string fn =
        (std::string)USER_CONSOLE_PREF_PATH + userInfo.username_ + "." + (std::string)USERS_PREFERENCES_FILETYPE;
...
//	(note: lastUpdateIndex==(unsigned int)-1 first time and returns as much as possible// nothing but
//lastUpdateClock)
//
//	format of xml:
//
...
void ConsoleSupervisor::insertMessageRefresh(HttpXmlDocument *xmlOut, const time_t lastUpdateCount,
  ...
  if (lastUpdateIndex > messages_.size() && lastUpdateIndex != (unsigned int)-1) {
    __SS__ << "Invalid lastUpdateIndex: " << lastUpdateIndex << " messagesArray size = " << messages_.size()
           << std::endl;
    __SS_THROW__;
  }
  ...
  else if (messages_[writePointer_].getTime())  // check that writePointer_ message has been initialized, therefore has
                                                // wrapped around at least once already
  {
  ...
  for (/*refreshReadPointer_=<first index to read>*/; refreshReadPointer_ != writePointer_;
    ...
    if (messages_[refreshReadPointer_].getCount() < lastUpdateCount) {
      if (!requestOutOfSync)  // record out of sync message once only
      {
        ...
        __SS__ << "Request is out of sync! Message count should be more recent than update clock! "
               << messages_[refreshReadPointer_].getCount() << " < " << lastUpdateCount << std::endl;
    ...
    for (refreshIndex_ = 0; refreshIndex_ < messages_[refreshReadPointer_].fields.size(); ++refreshIndex_)
      xmlOut->addTextElementToParent("message_" + messages_[refreshReadPointer_].fields[refreshIndex_].fieldName,
