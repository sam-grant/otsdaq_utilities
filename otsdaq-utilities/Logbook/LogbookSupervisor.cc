...
LogbookSupervisor::LogbookSupervisor(xdaq::ApplicationStub *stub)
    : CoreSupervisorBase(stub),
      allowedFileUploadTypes_({"image/png", "image/jpeg", "image/gif", "image/bmp", "application/pdf",
                               "application/zip", "text/plain"})  // init allowed file upload types
      ,
...
void LogbookSupervisor::init(void) {
  ...
  if (1)  // check if LOGBOOK_PATH and subpaths event exist?! (if not, attempt to create)
  {
    ...
    if (dir)
      ...
      std::stringstream ss;
      ss << __COUT_HDR_FL__ << "Service directory creation failed: " << path << std::endl;
      __SS_THROW__;
    }
...
void LogbookSupervisor::defaultPage(xgi::Input *in, xgi::Output *out) {
  ...
  *out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame src='/WebPath/html/Logbook.html?urn="
       << this->getApplicationDescriptor()->getLocalId() << "&active_experiment=" << activeExperiment_
       << "'></frameset></html>";
}
...
void LogbookSupervisor::forceSupervisorPropertyValues() {
  CorePropertySupervisorBase::setSupervisorProperty(
      CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.AutomatedRequestTypes, "RefreshLogbook");
  CorePropertySupervisorBase::setSupervisorProperty(
      CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.NonXMLRequestTypes, "LogImage | LogReport");
...
void LogbookSupervisor::request(const std::string &requestType, cgicc::Cgicc &cgiIn, HttpXmlDocument &xmlOut,
  ...
  //	{
  //		bool automaticCommand = requestType == "RefreshLogbook"; //automatic commands should not refresh cookie
  //code.. only user initiated commands should! 		bool checkLock = true;
  ...
  //pointer) 				,0//,&displayName			//acquire user's Display Name
...
//		Handles Web Interface requests to Logbook supervisor.
//		Does not refresh cookie for automatic update checks.
void LogbookSupervisor::nonXmlRequest(const std::string &requestType, cgicc::Cgicc &cgiIn, std::ostream &out,
                                      const WebUsers::RequestUserInfo &userInfo) {
  // Commands
  //	LogImage
  //	LogReport
  ...
  if (requestType == "LogImage") {
    ...
    out << "<!DOCTYPE HTML><html lang='en'><header><title>ots Logbook Reports</title></header><frameset col='100%' "
           "row='100%'><frame src='/WebPath/html/LogbookReport.html?urn="
        << this->getApplicationDescriptor()->getLocalId() << "&activeExperiment=" << activeExperiment
        << "'></frameset></html>";
  } else
    __COUT__ << "requestType request not recognized." << std::endl;
}
...
xoap::MessageReference LogbookSupervisor::MakeSystemLogbookEntry(xoap::MessageReference msg) {
  ...
  logPath = logDirPath + "/" + LOGBOOK_FILE_PREFACE + activeExperiment_ + "_" + (std::string)dayIndexStr +
...
//	__COUT__ << " Get Log Image " << src << std::endl;
//	*out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame
//src='/WebPath/html/LogbookImage.html?urn=" << 			this->getApplicationDescriptor()->getLocalId()
//<< "&src=" << src << "'></frameset></html>";
//}
//
////========================================================================================================================
////LogReport
...
void LogbookSupervisor::setActiveExperiment(std::string experiment) {
  ...
  if (activeExperiment_ != "" && activeExperiment_ != experiment)  // old active experiment is on its way out
    theRemoteWebUsers_.makeSystemLogbookEntry(allSupervisorInfo_.getGatewayDescriptor(),
...
void LogbookSupervisor::createExperiment(std::string experiment, std::string creator, HttpXmlDocument *xmlOut) {
  ...
  for (unsigned int i = 0; i < exps.size(); ++i)
    if (experiment == exps[i]) {
      if (xmlOut)
        xmlOut->addTextElementToData(XML_ADMIN_STATUS, "Failed - Experiment, " + experiment + ", already exists.");
      return;
    }
  ...
  if (directoryExists) {
      ...
      if (-1 == mkdir(dirPath.c_str(), 0755))  // make uploads directory
      {
        if (xmlOut)
          xmlOut->addTextElementToData(XML_ADMIN_STATUS,
                                       "Failed - uploads directory for " + experiment + " was not created.");
    ...
    xmlOut->addTextElementToData(XML_ADMIN_STATUS,
                                 "Directory already exists for " + experiment + ", re-added to list of experiments.");
    return;
  }
  ...
  if (-1 == mkdir(dirPath.c_str(), 0755) ||
    ...
    if (xmlOut)
      xmlOut->addTextElementToData(XML_ADMIN_STATUS, "Failed - directory, " + experiment + ", could not be created.");
    return;
  }
...
void LogbookSupervisor::webUserSetActiveExperiment(std::string experiment, HttpXmlDocument *xmlOut) {
  ...
  setActiveExperiment(experiment);
  if (xmlOut)
    xmlOut->addTextElementToData(XML_ADMIN_STATUS, "Active experiment set to " + experiment + " successfully.");
}
...
void LogbookSupervisor::removeExperiment(std::string experiment, std::string remover, HttpXmlDocument *xmlOut) {
  ...
  if (activeExperiment_ == experiment) setActiveExperiment();  // clear active experiment
...
void LogbookSupervisor::refreshLogbook(time_t date, unsigned char duration, HttpXmlDocument *xmlOut,
                                       std::ostringstream *out, std::string experiment) {
  ...
  if (!dir) {
    if (xmlOut)
      xmlOut->addTextElementToData(XML_STATUS, "Error - Directory for experiment, " + experiment + ", missing.");
  ...
  for (unsigned char i = duration; i != 0; --i) {
    ...
    entryPath =
        dirPath + "/" + LOGBOOK_FILE_PREFACE + experiment + "_" + (std::string)dayIndexStr + LOGBOOK_FILE_EXTENSION;
...
void LogbookSupervisor::savePostPreview(std::string &subject, std::string &text,
  ...
  for (unsigned int i = 0; i < files.size(); ++i) {
    ...
    {
      if (xmlOut)
        xmlOut->addTextElementToData(XML_STATUS, "Failed - invalid file type, " + files[i].getDataType() + ".");
      return;
    }
    ...
    filename =
        previewPath + "/" + (std::string)LOGBOOK_PREVIEW_UPLOAD_PREFACE + (std::string)fileIndex + "." + filename;
...
//      if not approve
//          delete directory
void LogbookSupervisor::movePreviewEntry(std::string previewNumber, bool approve, HttpXmlDocument *xmlOut) {
  ...
  if (approve) {
    ...
    for (unsigned int i = 0; i < fileTypes.size(); ++i) {
      ...
      logFilename = (std::string)LOGBOOK_PREVIEW_UPLOAD_PREFACE + entryTimeLabel + "_" + (std::string)fileIndex + "." +
                    fileExtension;
...
void LogbookSupervisor::hideLogbookEntry(const std::string &entryId, bool hide, const std::string &hider) {
  ...
  std::string logDirPath = (std::string)LOGBOOK_PATH + (std::string)LOGBOOK_LOGBOOKS_PATH +
                           (std::string)LOGBOOK_EXPERIMENT_DIR_PREFACE + activeExperiment_;
  std::string logPath = logDirPath + "/" + LOGBOOK_FILE_PREFACE + activeExperiment_ + "_" + (std::string)dayIndexStr +
                        LOGBOOK_FILE_EXTENSION;
  ...
  xercesc::DOMElement *hiddenParentEl,
      *entryParentEl = logXml.getMatchingElement(XML_LOGBOOK_ENTRY, i);  // get entry element
  ...
  if (hide)  // remove entry
  {
    ...
    hiddenParentEl = logXml.addTextElementToParent(XML_LOGBOOK_ENTRY_HIDDEN, "1",
