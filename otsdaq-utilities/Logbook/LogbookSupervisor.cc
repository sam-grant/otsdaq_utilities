...
void LogbookSupervisor::defaultPage (xgi::Input *in, xgi::Output *out)
{
	...
	*out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame src='/WebPath/html/Logbook.html?urn=" << this->getApplicationDescriptor ()->getLocalId () << "&active_experiment=" << activeExperiment_ << "'></frameset></html>";
}
...
{
	CorePropertySupervisorBase::setSupervisorProperty (CorePropertySupervisorBase::SUPERVISOR_PROPERTIES.UserPermissionsThreshold, std::string () + "*=1 | CreateExperiment=-1 | RemoveExperiment=-1 | GetExperimentListAdmin=-1 | SetActiveExperiment=-1" + " | AdminRemoveRestoreEntry=-1");
}
...
void LogbookSupervisor::request (const std::string &requestType, cgicc::Cgicc &cgiIn, HttpXmlDocument &xmlOut, const WebUsers::RequestUserInfo &userInfo)
{
	//Commands
	//	CreateExperiment
	//	RemoveExperiment
	...
	else if (requestType == "PreviewEntry")
	{
		...
		__COUT__ << "EntryText " << EntryText << std::endl
		         << std::endl;
...
//		Handles Web Interface requests to Logbook supervisor.
//		Does not refresh cookie for automatic update checks.
void LogbookSupervisor::nonXmlRequest (const std::string &requestType, cgicc::Cgicc &cgiIn, std::ostream &out, const WebUsers::RequestUserInfo &userInfo)
{
	//Commands
	//	LogImage
	//	LogReport
	...
	if (requestType == "LogImage")
	{
		...
		out << "<!DOCTYPE HTML><html lang='en'><frameset col='100%' row='100%'><frame src='/WebPath/html/LogbookImage.html?urn=" << this->getApplicationDescriptor ()->getLocalId () << "&src=" << src << "'></frameset></html>";
	}
	else if (requestType == "LogReport")
	{
		...
		out << "<!DOCTYPE HTML><html lang='en'><header><title>ots Logbook Reports</title></header><frameset col='100%' row='100%'><frame src='/WebPath/html/LogbookReport.html?urn=" << this->getApplicationDescriptor ()->getLocalId () << "&activeExperiment=" << activeExperiment << "'></frameset></html>";
	}
	else
		__COUT__ << "requestType request not recognized." << std::endl;
}
...
void LogbookSupervisor::createExperiment (std::string experiment, std::string creator, HttpXmlDocument *xmlOut)
{
	...
	if (directoryExists)
	{
		...
		xmlOut->addTextElementToData (XML_ADMIN_STATUS, "Directory already exists for " + experiment + ", re-added to list of experiments.");
		return;
	}
...
void LogbookSupervisor::removeExperiment (std::string experiment, std::string remover, HttpXmlDocument *xmlOut)
{
	...
	fprintf (fp, "%s -- %s Experiment removed by %s.\n", asctime (localtime (&((time_t const &)(time (0))))), experiment.c_str (), remover.c_str ());
	fclose (fp);
...
void LogbookSupervisor::refreshLogbook (time_t date, unsigned char duration, HttpXmlDocument *xmlOut, std::ostringstream *out, std::string experiment)
{
	...
	while ((entry = readdir (dir)))  //loop through all entries in directory and remove anything expired
	{
		if (strcmp (entry->d_name, ".") != 0 && strcmp (entry->d_name, "..") != 0 && strcmp (entry->d_name, ".svn") != 0)
		{
			...
			if ((time (0) - dirCreateTime) > LOGBOOK_PREVIEW_EXPIRATION_TIME)
			{
				...
				__COUT__ << "rm -rf " << previewPath + (std::string)entry->d_name << std::endl
				         << std::endl;
...
void LogbookSupervisor::savePostPreview (std::string &subject, std::string &text, const std::vector<cgicc::FormFile> &files, std::string creator, HttpXmlDocument *xmlOut)
{
	...
	for (unsigned int i = 0; i < files.size (); ++i)
	{
		...
		{
			if (xmlOut) xmlOut->addTextElementToData (XML_STATUS, "Failed - invalid file type, " + files[i].getDataType () + ".");
			return;
		}
