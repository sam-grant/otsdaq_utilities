	...
	std::thread(
	    [](ControlsDashboardSupervisor* cs) {
		    ...
		    while(true)
		    {
			              ...
			              .end())  // We have here current list of PV Dependencies
			    {
				    for(auto pv : mapReference->second)
				    {
					    ...
					    __COUT__ << "THREAD actual time: " << std::time(NULL)
					             ...
					             << cs->uidPollTimeMap_.at(mapReference->first) +
					                    10 * refreshRate
					             << " seconds" << std::endl;
					    if(std::time(NULL) >
			    ...
			    sleep(minTime);
			    __COUT__ << "Loop over pvs subscribing - waiting time: " << minTime
			             << " seconds" << std::endl;
		    }
	    },
	    this)
                                                    ...
                                                    HttpXmlDocument& xmlOut)
{
	...
	if(pvList.size() > 0)
	{
		while((nextPos = pvList.find(",", pos)) != std::string::npos)
		{
			...
			__SUP_COUT__ << pv << std::endl;
			std::array<std::array<std::string, 5>, 10> pvInformation =
