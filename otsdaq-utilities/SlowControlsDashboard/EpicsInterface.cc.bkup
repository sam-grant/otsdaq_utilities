...
void EpicsInterface::eventCallback(struct event_handler_args eha)
{
	...
	if(eha.status == ECA_NORMAL) {
		...
		if(DEBUG){printf("channel %s: ", ca_name(eha.chid));}

		switch (eha.type){
		case DBR_CTRL_CHAR:
			if(true){std::cout << "Response Type: DBR_CTRL_CHAR" << std::endl;}
...
{
	if(DEBUG){std::cout << "EpicsInterface::checkIfPVExists(): PV Info Map Length is " << mapOfPVInfo_.size() << std::endl;}
...
{
	if(true){std::cout << "EpicsInterface::getControlValues(" << pvName << ")" << std::endl;}
	if(!checkIfPVExists(pvName))
	{
		std::cout << pvName << " doesn't exist!" << std::endl;
		return;
...
//		Sleep(1);
//	if(i==2)
//		throw std::runtime_error("Channel failed for " + pvName);
...
{
	if(DEBUG){std::cout << "Reading Control Values from " << pvName << "!" << std::endl;}
