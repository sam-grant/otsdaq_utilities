...
ECLSupervisor::ECLSupervisor(xdaq::ApplicationStub* s)
    : CoreSupervisorBase(s),
      ...
      supervisorConfigurationPath_("/" + supervisorContextUID_ + "/LinkToApplicationTable/" +
...

{
  // try
  {
    // theConfigurationGroupKey_ =
    // theConfigurationManager_->makeTheConfigurationGroupKey(atoi(SOAPUtilities::translate(theStateMachine_.getCurrentMessage()).getParameters().getValue("ConfigurationGroupKey").c_str()));
    ...
    ConfigurationTree configLinkNode =
        theConfigurationManager_->getSupervisorConfigurationNode(supervisorContextUID_, supervisorApplicationUID_);
  ...
  // catch(...)
  //{
  //	{__SS__;__THROW__(ss.str()+"Error configuring the visual supervisor most likely a plugin name is wrong or your
  //configuration table is outdated and doesn't match the new plugin definition!");}
  //}
}
...
int ECLSupervisor::Write(WriteState state) {
  ...
  if (state != WriteState::kStart && state != WriteState::kResume) {
    ...
    dur_ss << std::setw(2) << std::setfill('0') << dur_h << ":" << std::setw(2) << std::setfill('0') << dur_m << ":"
           << std::setw(2) << std::setfill('0') << dur_s << "." << dur;
